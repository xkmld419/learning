...
##	设计思路：
##	2，使用方式，
##				构造：MessageQueueEx *p = new MessageQueueEx("/home/bill");
##				判断：if (!p || !(*p)) throw 1;
##				使用...
##
*/
...
void MessageQueueEx::open(bool ifblock, bool ifcreate, int maxlen, int maxnum)
{
	m_iBlock = ifblock;
	m_iMaxNum = maxnum;
	m_iMaxLen = maxlen;
...
int MessageQueueEx::send(void* data,long lType,int len)
{
	//##Ub@o5DJ5OV2;?<BGSEOH<6
	int ret;
...
__SEND_TYPE:
	...
	if (ret == -1)
	{
		...
		if (m_iErrorNo == E_INTR) {
			Log::log (1000, "recv signal....");
			goto __SEND_TYPE;
		}
		Log::log(0,"Type:%ld,Len:%d",lType,len);

		THROW(MBC_MessageQueue+200+m_iErrorNo); //		return -1;
	}

	return len;
}

int MessageQueueEx::receive(char* pbuf,long lType,unsigned int iLen)
{
	int ret;
	unsigned int iRecvSize=0;
	if(m_pBuffEx==0)
	{
		m_iBuffExSize = iLen +sizeof(long);
		m_pBuffEx =(char*) malloc(m_iBuffExSize);
		if(0== m_pBuffEx)
			THROW(MBC_MessageQueue+12);
	}else
	{
		if((iLen>(m_iBuffExSize-sizeof(long)))&&(iLen<m_iMaxLen*m_iMaxNum))
		{
			m_iBuffExSize = iLen +sizeof(long);
			m_pBuffEx = (char*)realloc(m_pBuffEx,m_iBuffExSize);
			if(0== m_pBuffEx)
				THROW(MBC_MessageQueue+701);
		}else
		{
			if(iLen>m_iMaxLen*m_iMaxNum)
				THROW(MBC_MessageQueue+12); 
		}
	}
	iRecvSize = iLen+sizeof(long);
#ifdef SYSTEM_V

	memset ((char *)m_pBuffEx, 0, m_iBuffExSize);

__RECEIVE_TYPE:

	ret = msgrcv (m_iSysMQID, m_pBuffEx, iRecvSize, lType, (m_iBlock ? MSG_NOERROR : IPC_NOWAIT|MSG_NOERROR));

	if (ret >=0)
	{
		memcpy (pbuf, (char *)m_pBuffEx+sizeof(long), ret);
	}
	else
	{
		m_iErrorNo = _getStandardError (errno);
...
__RECEIVE_TYPE:
	ret= mq_receive (m_iSysMQID, pbuf, /*attr.mq_msgsize*/iLen, NULL);
...
int MessageQueueEx::send(void *data, int len)
{
	//##这里的实现不考虑优先级
	int ret;
	//cout<<"Send:"<<this->m_iSysMQID<<endl;
...
int MessageQueueEx::receive(char* pbuf,unsigned int iLen)
{
	//cout<<"Recv:"<<this->m_iSysMQID<<endl;
	int ret;
	unsigned int iRecvSize=0;
	if(m_pBuffEx==0)
	{
...
__RECEIVE:
	...
	ret = msgrcv (m_iSysMQID, m_pBuffEx, iRecvSize, 0, (m_iBlock ? MSG_NOERROR : IPC_NOWAIT|MSG_NOERROR));
...
int MessageQueueEx::receive(char *buf)
{
	//##如果没有消息，返回空
	/*
	2005-05-24
...
int MessageQueueEx::getMessageNumber()
{
	if (!m_bAttached)
	{
		m_iErrorNo = E_CLOSE;
...
//MessageQueueEx::MessageQueueEx(char *mqname)
//##ModelId=4200B83B0109
MessageQueueEx::MessageQueueEx(int key)
{
...
int MessageQueueEx::getSysID()
{
	return m_iSysMQID;
}

//##ModelId=42017CA40228
int MessageQueueEx::getOccurPercent()
{
...
int MessageQueueEx::_getStandardError(int err)
{
	/*
	SYSTEM_V                                         POSIX
