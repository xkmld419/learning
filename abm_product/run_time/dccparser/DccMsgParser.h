...
public:
   ...
   private:
    ...
    template<class T> int _getAvpValue(char *avp_name1, T &data);
		...
		void setOutStr(std::string& avp_name, char *sFormatStr, const char *sTmpBuf);
...
//		std::map<std::string, std::string>    	mapStr;
		...
		std::string	m_sFailedAvp;
    		...
    		////性能测试协议解析
public:
		char m_sMsgBuf[20480];
		char *m_pCurr;
		int  m_iCurrLen;
		int  m_iMsgLen;
		
		int  setMsgBuf(std::string &msg_buf);
                int  setMsgBuf(char *pBuf, int iLen);
	    void  getMsgBuf(std::string &msg_buf);
            void  getMsgBuf(char *pBuf, int *piLen);
