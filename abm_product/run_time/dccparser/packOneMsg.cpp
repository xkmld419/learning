...
int main(int argc,char* argv[])
{
    ...
    */
	...
	int i=0;
    i=poOcpMsgParser->getGroupedAvpNums("Service-Information");
	for(int j=0;j<i;j++)
	{
	subCCAinfo *psubCCAinfo = new subCCAinfo;
	itent=poOcpMsgParser->getAvpValue("Service-Information.Request-Id",sVar);
	strncpy((char*)psubCCAinfo->Request_Id,sVar.c_str(),sizeof(*psubCCAinfo->Request_Id));	

	itent=poOcpMsgParser->getAvpValue("Service-Information.Destination-Type",iVar);
    psubCCAinfo->Destination_Type=iVar;

	itent=poOcpMsgParser->getAvpValue("Service-Information.Destination-Account",sVar);
	strncpy((char*)psubCCAinfo->Destination_Account,sVar.c_str(),sizeof(*psubCCAinfo->Destination_Account));	
	
	itent=poOcpMsgParser->getAvpValue("Service-Information.Destination-Attr",iVar);
    psubCCAinfo->Destination_Attr=iVar;

	itent=poOcpMsgParser->getAvpValue("Service-Information.Service-Platform-ID",sVar);
	strncpy((char*)psubCCAinfo->Service_Platform_ID,sVar.c_str(),sizeof(*psubCCAinfo->Service_Platform_ID));
	
	itent=poOcpMsgParser->getAvpValue("Service-Information.QueryType",iVar);
    psubCCAinfo->QueryType=iVar;
	test.ttvecC.push_back(*psubCCAinfo);
    }
	printf("test,%s",test.ttvecC[0].Request_Id);
	printf("test",test);
