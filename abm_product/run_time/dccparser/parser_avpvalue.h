...
/*                                                                        */
/* END_COPYRIGHT                                                          */
/* $Id: parser_avpvalue.h,v 1.24 2005/03/18 14:24:41 vfajardo Exp $ */
...
ntohq(ACE_UINT64 q)
{
  #if !defined(OS_LINUX) && !defined(DEF_LINUX)
  	return q;
  #else
	  ACE_UINT64 int64;
	  char buf1[9], buf2[9];
...
//  friend class Singleton<Integer64Parser, Null_Mutex>;
  public:
    ...
    void parseAppToRaw() throw(AAAErrorStatus)
    {
      AAAMessageBlock* aBuffer;//结构体到AVP 的数据存储区间
      AAAInteger64AvpContainerEntry *e;//64位容器实例的变量
      getRawData(aBuffer);//绑定数据
      getAppData(e);//绑定容器
      if (e->dataType() != AAA_AVP_INTEGER64_TYPE &&
          e->dataType() != AAA_AVP_UINTEGER64_TYPE)
      {
...
class OctetstringParser : public AvpValueParser
{
  public:
    ...
    void parseAppToRaw() throw(AAAErrorStatus)//结构体转到avp
    {
      AAAMessageBlock* aBuffer = getRawData();
      AAAStringAvpContainerEntry *e;
      getAppData(e);
...
class _avp_parse_type 
{
  public:
     static AnyParser *p_AnyParser;
     static Integer32Parser *p_Integer32Parser;
     static Integer64Parser *p_Integer64Parser;
     static OctetstringParser *p_OctetstringParser;
     static Utf8stringParser *p_Utf8stringParser;
     static AddressParser *p_AddressParser;
     static IPFilterRuleParser *p_IPFilterRuleParser;
     static GroupedParser *p_GroupedParser;
     static DiamuriParser *p_DiamuriParser;
  public:
     static AvpValueParser* getParserPiontor(int type){
         AAAErrorStatus st;
         switch(type) 
         {
