...
const   int MAX_PARAMS_COUNT = 150;     //清单表结构大约100个字段 3.28修改
...
public:
	TOCIException(sword errNumb, OCIError *err, char const *cat, char const *sql);//执行OCI函数发生的错误
...
public:
	...
	TOCIField& field(char const *fieldName) ;						//根据列名(不分大小写)返回列信息; 建议使用Field(int i)获得更高的效率
