...
template <typename MSGBUF >
void MQ<MSGBUF>::setKey(key_t key)
{
	msgKey = key;
	ifOpen = open();
	checkopen();
...
template <typename MSGBUF >
void MQ<MSGBUF>::cpyMsg(MSGBUF* mb)
{
...
template <typename MSGBUF >
MSGBUF* MQ<MSGBUF>::getMsg() 
{
	return &msgbuf;
};

template <typename MSGBUF >
void MQ<MSGBUF>::checkBuf() 
{
...
template <typename MSGBUF >
bool MQ<MSGBUF>::receive(long mtype) 
{
	int ret;
