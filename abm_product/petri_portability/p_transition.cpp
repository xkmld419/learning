...
transition::load()
{
	...
	try {
		...
		if (query.next()){
			...
			l_execObjID = query.field("TRANSITION_OBJ_ID").asLong();
...
int transition::AutoPlaceSendMessage(long transID,const string& inState,string transName)
{
	...
	for (i=0;i<ts.m_size;i++)	
	{
		if(getExecAttr() == 0)
		{
			...
			{
				Date now;
				sprintf(tempChar,"%s¶ÔÓ¦µÄ±äÇ¨IDÎª%d,µ±Ç°×´Ì¬ÎªÕýÔÚÔËÐÐ£¬ÔËÐÐÊ±¼äÎª%s",transName,transID,now.toString());
			...
			{
				Date now;
				sprintf(tempChar,"%s¶ÔÓ¦µÄ±äÇ¨IDÎª%d,µ±Ç°×´Ì¬ÎªÒÑ¾­½áÊø£¬½áÊøÊ±¼äÎª%s",transName,transID,now.toString());
				sprintf(sql, "select STAFF_ID,ACC_NBR,SERV_ID from P_GROUP_STAFF where STAFF_GROUP_ID = '%d'",ts.m_STAFF_GROUP_ID[i]);
				qry.setSQL (sql);
				qry.open ();
				while (qry.next())
			...
			{
				Date now;
				sprintf(tempChar,"%s¶ÔÓ¦µÄ±äÇ¨IDÎª%d,µ±Ç°×´Ì¬ÎªÒÑ¾­³ö´í£¬³ö´íÊ±¼äÎª%s",transName,transID,now.toString());
				sprintf(sql, "select STAFF_ID,ACC_NBR,SERV_ID from P_GROUP_STAFF where STAFF_GROUP_ID = '%d'",ts.m_STAFF_GROUP_ID[i]);
				qry.setSQL (sql);
				qry.open ();
				while (qry.next())
