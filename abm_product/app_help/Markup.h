...
#define MARKUP_STL     //add ziwei
#define MARKUP_STDCONV //add ziwei
...
#pragma once
	...
	#pragma warning(disable:4786) // string long names
	#endif // VC++ 2005 STL
#else // not VC++
...
//
	...
	#include <string>
	using namespace std; //add ziwei
	#if defined(MARKUP_WCHAR)
			#define MCD_STR wstring
	#else // not MARKUP_WCHAR
			#define MCD_STR string
	#endif // not MARKUP_WCHAR
...
struct MCD_CSTR_FILENAME
{
	...
	MCD_CSTR_FILENAME( const string& s ) { pcsz = s.c_str(); };
