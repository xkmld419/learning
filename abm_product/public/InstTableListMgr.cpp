...
bool InstTableListMgr::getSpecialTableName(int i_iBillingCycleID,
                                    int i_iBillFlowID,
                                    int i_iTableType,
                                    int i_iOrgID,
                                    char * o_sTableName,
                                    char* o_sTableNameRB , bool ifRead)
{
	if(i_iBillingCycleID==0 || i_iBillFlowID==0 || i_iOrgID==0)
	{
		Log::log(0,"警告：无法找到BillingCycleID=%d,BillFLowID=%d,TableType=%d,OrgID=%d 对应的表",i_iBillingCycleID,i_iBillFlowID,i_iTableType,i_iOrgID);
		Log::log(0,"警告：返回TableType=%d 对应的基表",i_iTableType);
		switch(i_iTableType)
		{
			case TABLE_TYPE_ENDFILE:
			     strcpy(o_sTableName,"END_FILE_LOG");
			     break;
			
			case TABLE_TYPE_ACCT_ITEM_AGGR:
			     strcpy(o_sTableName,"ACCT_ITEM_AGGR");
			     break;
			case TABLE_TYPE_ACCT_ITEM_AGGR_RB	:
			     strcpy(o_sTableName,"ACCT_ITEM_AGGR_RB");
			     break;
			
			case TABLE_TYPE_EVENT_AGGR:
			     strcpy(o_sTableName,"EVENT_AGGR");
			     break;
			case TABLE_TYPE_EVENT_AGGR_RB:
			     strcpy(o_sTableName,"EVENT_AGGR_RB");
			     break;
			...
			case TABLE_TYPE_EVENT_PRICE:
			     strcpy(o_sTableName,"EVENT_PRICE");
			     break;
			
			case TABLE_TYPE_CYCLE_INST:
			     strcpy(o_sTableName,"CYCLE_INST");
			     break;
			
			case TABLE_TYPE_ACCT_DISCT_DETAIL:
			     strcpy(o_sTableName,"ACCT_DISCT");
			     break;
			case TABLE_TYPE_ACCT_DISCT_DETAIL_RB:
			     strcpy(o_sTableName,"ACCT_DISCT_RB");
			     break;
			
			case TABLE_TYPE_ACCT_DISCT_AGGR:
			     strcpy(o_sTableName,"ACCT_DISCT_AGGR");
			     break;
			
			case EVENT_TABLE_TYPE_EXTEND_EVENT:
			     strcpy(o_sTableName,"EXTEND_EVENT");
			     break;
			
			case 21:
			     strcpy(o_sTableName,"MOBILE_CALL_EVENT");
			     break;
			case 22:
			     strcpy(o_sTableName,"MOBILE_DATA_EVENT");
			     break;
			case 23:
			     strcpy(o_sTableName,"MOBILE_VALUE_EVENT");
			     break;
			case 24:
			     strcpy(o_sTableName,"MOBILE_SMS_EVENT");
			     break;
			
			case EVENT_TABLE_TYPE_CALL_EVENT:
			     strcpy(o_sTableName,"CALL_EVENT");
			     break;
			case EVENT_TABLE_TYPE_DATA_EVENT:
			     strcpy(o_sTableName,"DATA_EVENT");
			     break;
			case EVENT_TABLE_TYPE_VALUE_ADDED_EVENT:
			     strcpy(o_sTableName,"VALUE_ADDED_EVENT");
			     break;
			
			case EVENT_TABLE_TYPE_CYCLE_FEE_EVENT:
			     strcpy(o_sTableName,"CYCLE_FEE_EVENT");
			     break;
			case EVENT_TABLE_TYPE_DISCT_EVENT:
			     strcpy(o_sTableName,"DISCT_EVENT");
			     break;
			case EVENT_TABLE_TYPE_SET_FEE_EVENT:
			     strcpy(o_sTableName,"SET_FEE_EVENT");
			     break;
			
			case EVENT_TABLE_TYPE_OTHER_EVENT:
			     strcpy(o_sTableName,"OTHER_EVENT");
			     break;
			
			case EVENT_TABLE_TYPE_REDO_EVENT:
			case EVENT_TABLE_TYPE_NOOWNER_EVENT:
			case EVENT_TABLE_TYPE_DISUSE_EVENT:
		  default :
			     strcpy(o_sTableName,"REDO_EVENT");
			     break;
		}
    	...
    	if(getRBTableType(i_iTableType,nRbTableType,ifRead))
    	{
    		if(nRbTableType != i_iTableType)
    		{
    			getSpecialTableName(i_iBillingCycleID,i_iBillFlowID,nRbTableType,i_iOrgID,o_sTableNameRB,NULL,ifRead);
    		}
    	}
    }
