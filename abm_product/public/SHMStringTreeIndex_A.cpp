...
int SHMStringTreeIndex_A::getUsedSize()
{
	int  size = (*m_piListLen ) * (*m_piListUsed) + \
				(*m_piListUsed)*sizeof(SHMStringList_A) + \
...
unsigned long  SHMStringTreeIndex_A::getMemAdress()
{		
	if (m_poSHM)
	{
...
time_t  SHMStringTreeIndex_A ::getLoadTime()
{
	time_t shm_ctime;
...
unsigned int SHMStringTreeIndex_A::getMemSize()
{
	int  size = (*m_piListLen ) * (*m_piListTotal)  + \
				(*m_piTreeTotal)*sizeof(SHMStringList_A) + \
				16*sizeof(unsigned int);
