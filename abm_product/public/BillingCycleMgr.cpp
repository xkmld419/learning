...
{ 
	        unsigned int iOffset;
        BillingCycle * pResult;
        static int giBillingCycleID=0;
        static int giOffset =0;
        pResult = *m_poCycleData;
        if(giBillingCycleID==iBillingCycleID && giOffset!=0){
                return pResult+giOffset;
        }
        int iCount = m_poCycleData->getCount();
        for (iOffset=1; iOffset<=iCount; iOffset++) {
                if (pResult[iOffset].m_iBillingCycleID == iBillingCycleID){
                        giBillingCycleID=iBillingCycleID;
                        giOffset=iOffset;
                        return pResult+iOffset;
                }
        }
...
BillingCycle *BillingCycleMgr::getPreBillingCycle(int iBillingCycleID)
{
	...
	pResult = *m_poCycleData;
  int iCnt = m_poCycleData->getCount();
	for (iOffset=1; iOffset<=iCnt; iOffset++) {
		if (pResult[iOffset].m_iBillingCycleID == iBillingCycleID) {
