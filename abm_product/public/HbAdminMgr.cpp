...
int HbAdminMgr::getConfInfo(const char * sIniHeader)
{
	...
	sprintf (m_sTIBS_HOME, "/bill/app/TIBS_HOME/");
...
int HbAdminMgr::CutString(const char * sInputUserInfo)
{
	char tempUserName[20] ={0};
	char tempPassSWord[20]= {0};
	int  i = 0 ,j = 0 ;	
		
	if (sInputUserInfo == NULL) return 0;

	while (*sInputUserInfo != '\0')
	{
		if (*sInputUserInfo == '/')	break;		
		tempUserName[i++] = *sInputUserInfo++;
	}	 	
	while (*sInputUserInfo++ != '\0')	
	{	
		tempPassSWord[j++] = *sInputUserInfo;
	}	
	memset(m_sLogUserName,0,sizeof(m_sLogUserName));
	memset(m_sLogPassWord,0,sizeof(m_sLogPassWord));	
