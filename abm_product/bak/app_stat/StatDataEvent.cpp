...
StatDataEvent::StatDataEvent():TStatClass(OID_DATA_EVENT)
{
    pRead = NULL;
}

StatDataEvent::~StatDataEvent()
{
    if ( pRead)
       delete pRead;
	pRead = NULL;
...
POINT_TYPE StatDataEvent::GetMaxPoint(char * sTableName,POINT_TYPE Point)
{
...
POINT_TYPE StatDataEvent::GetMinPoint(char * sTableName,POINT_TYPE Point)
{
	TOCIQuery qry(&gpDBLink);
	char sSql[200];
	POINT_TYPE 	Result;
...
void StatDataEvent::OpenCursor(char *sTableName,POINT_TYPE MinPoint,POINT_TYPE MaxPoint)
{
	char 	sSql[200];
	
	if (pRead == NULL)
        pRead = new TBL_DATA_EVENT_CLASS();
...
bool StatDataEvent::GetRecordFromTable(POINT_TYPE &BreakPoint)		
{
	char sTemp[200];
...
void StatDataEvent::CloseCursor()
{		
	if (pRead){
		delete pRead;
        pRead=NULL;
...
int StatDataEvent::GetExprFieldValue( int iFieldID, TFieldValue * pTFieldValue)
{
    return -1;
}

int  StatDataEvent::GetFieldValue(int iFieldID,TFieldValue * pTFieldValue)
{
	char	sTemp[200];
	int		iValue;
	class OrgMgr orgmgr;
