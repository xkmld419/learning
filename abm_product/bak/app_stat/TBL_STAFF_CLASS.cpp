...
bool	TBL_STAFF_CLASS::Next()
{
		...
		try{
			...
			item.ORGAN_ID=m_qry->field("ORGAN_ID").asLong();
			item.PARENT_PARTYROLEID=m_qry->field("PARENT_PARTYROLEID").asLong();
			item.BSS_ORG_ID=m_qry->field("BSS_ORG_ID").asLong();
			item.WORK_GROUP_ID=m_qry->field("WORK_GROUP_ID").asLong();
		return true;
		}
		catch (TOCIException &oe) {
			cout<<"Error occured ... in TBL_STAFF_CLASS.cpp"<<endl;
...
{
		if ( strlen(sParam)>15 ) 
			 throw TException( " Set_STAFF_CODE  string over 15!");
...
{
		if ( strlen(sParam)>30 ) 
			 throw TException( " Set_PASSWD  string over 30!");
...
void	TBL_STAFF_CLASS::Set_STAFF_DESC(char	* sParam)
{
		if ( strlen(sParam)>250 ) 
			 throw TException( " Set_STAFF_DESC  string over 250!");
...
void	TBL_STAFF_CLASS::SetBuf(int iBufLen)
{
		iBufferSize=iBufLen;
...
MEM_STAFF_CLASS::MEM_STAFF_CLASS(char *sTableName)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_STAFF_CLASS::Load(char *sTableName)
{
