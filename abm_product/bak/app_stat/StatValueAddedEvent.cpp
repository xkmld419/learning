...
StatValueAddedEvent::StatValueAddedEvent():TStatClass(OID_VALUE_ADDED_EVENT)
{
    pRead = NULL;
}

StatValueAddedEvent::~StatValueAddedEvent()
{
    if ( pRead)
       delete pRead;
	pRead = NULL;
...
POINT_TYPE StatValueAddedEvent::GetMaxPoint(char * sTableName,POINT_TYPE Point)
{
...
POINT_TYPE StatValueAddedEvent::GetMinPoint(char * sTableName,POINT_TYPE Point)
{
	TOCIQuery qry(&gpDBLink);
	char sSql[200];
	POINT_TYPE 	Result;
...
void StatValueAddedEvent::OpenCursor(char *sTableName,POINT_TYPE MinPoint,POINT_TYPE MaxPoint)
{
	char 	sSql[200];
	
	if (pRead == NULL)
        pRead = new TBL_VALUE_ADDED_EVENT_CLASS();
...
bool StatValueAddedEvent::GetRecordFromTable(POINT_TYPE &BreakPoint)		
{
	char sTemp[200];
...
void StatValueAddedEvent::CloseCursor()
{		
	if (pRead){
		delete pRead;
        pRead=NULL;
...
int  StatValueAddedEvent::GetCycle()
{
...
int StatValueAddedEvent::GetExprFieldValue( int iFieldID, TFieldValue * pTFieldValue)
{
    return -1;
}

int  StatValueAddedEvent::GetFieldValue(int iFieldID,TFieldValue * pTFieldValue)
{
	char	sTemp[200];
	int		iValue;
	class OrgMgr orgmgr;
    ...
    switch ( iFieldID){
    	...
    	*/
    	...
    	case	FA_ADDVALUE_ORG_LEVELID100:
    		...
    		break;
    	case	FA_ADDVALUE_ORG_LEVELID200:
    		{
    			int iOrgID=pRead->Get_BILLING_ORG_ID();
    			int iOrgLevelID=-1;
    		...
    		break;
    	case	FA_ADDVALUE_ORG_LEVELID300:
    		{
    			int iOrgID=pRead->Get_BILLING_ORG_ID();
    			int iOrgLevelID=-1;
    		...
    		break;
    	case	FA_ADDVALUE_ORG_LEVELID400:
    		{
    			int iOrgID=pRead->Get_BILLING_ORG_ID();
    			int iOrgLevelID=-1;
    		...
    		break;
    	case	FA_ADDVALUE_ORG_LEVELID500:
    		{
    			int iOrgID=pRead->Get_BILLING_ORG_ID();
    			int iOrgLevelID=-1;
