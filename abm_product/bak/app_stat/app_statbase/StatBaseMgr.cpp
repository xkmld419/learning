	...
	try{
		...
		for(int i=1; i<g_argc; i++)
		{
			...
			*/
			...
			{
				printf("不支持的参数选项，请参考sgwstat -a\n\n");
				return -1;
			}
		}
		if(!pDraw)
		{
			//printf("不支持的参数选项，请参考sgwstat -h\n\n");
			PrintUsage();
			return -1;
		}
		//pDraw->StartWin();
...
"		-t		每隔秒后重复选项                                          \n"
"		-p		显示当天的统计信息，内容至少包括：                        \n"
"				（请求数、请求成功数、数据量、能力调用次数、调用成功数）  \n"
"		-P		显示自启动以来累计的统计信息，内容至少包括：（请求数、    \n"
"				请求成功数、数据量、能力调用次数、调用成功数）            \n"
"		-o		显示自当前小时整点以来累计的统计信息，内容至少包括：      \n"
"				（请求数、请求成功数、数据量、能力调用次数、调用成功数）  \n"
"		-r		显示系统实时吞吐量，内容至少包括（积压数量、              \n"
"				当前吞吐量（每个时间单位的性能））                        \n"
"		-m		[module]进程线程分类名称                                  \n"
"		-Q		显示SGW系统消息队列的使用情况                             \n"
"		-L		显示日志文件的使用情况                                    \n"
"		-u		显示今日凌晨到目前系统用户登录情况                        \n"
"		-ses		显示系统进程/线程情况，含义同sgwprocess –a                \n"
"		-seg		显示当前内存的使用状况，含义同sgwshm –stat                \n"
"		-ca		创建Stat内存供统计业务进程信息使用，必须创建         \n"
"		-de		删除stat内存                                          \n");
}
