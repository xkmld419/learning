/*VER: 1*/ 
#ifndef _TBL_CALL_EVENT_CLASS_H__
#define _TBL_CALL_EVENT_CLASS_H__
#include <iostream>
#include "TOCIQuery.h"
#include <string>
#include <vector>
...
typedef struct _TBL_CALL_EVENT_STR_{
	long	EVENT_ID;
	long	BILL_FLOW_ID;
	...
	long	SWITCH_ID;
	char	TRUNK_IN[13];
	long	TRUNK_IN_TYPE_ID;
	char	TRUNK_OUT[13];
	long	TRUNK_OUT_TYPE_ID;
	long	FILE_ID;
	...
	void	Print(ostream& os) const {
			...
			os << "TRUNK_IN_TYPE_ID:["<<TRUNK_IN_TYPE_ID<<"]"<<endl;
			os << "TRUNK_OUT:["<<TRUNK_OUT<<"]"<<endl;
			os << "TRUNK_OUT_TYPE_ID:["<<TRUNK_OUT_TYPE_ID<<"]"<<endl;
	...
	void	ClearBuf(void){
			memset(&EVENT_ID,0,sizeof(_TBL_CALL_EVENT_STR_));
	};
}_TBL_CALL_EVENT_STR_;

inline ostream& operator<< (ostream& os, const _TBL_CALL_EVENT_STR_ &obj) {
	obj.Print(os);
	return os;
}
...
class TBL_CALL_EVENT_CLASS{
	friend class MEM_CALL_EVENT_CLASS;
	private:
		_TBL_CALL_EVENT_STR_		item;
		_TBL_CALL_EVENT_STR_		*a_item;
		int		iCurPos;
		TOCIQuery	*		m_qry;
	...
	public:
		...
		long	Get_TRUNK_IN_TYPE_ID(void);
		char*	Get_TRUNK_OUT(void);
		long	Get_TRUNK_OUT_TYPE_ID(void);
		...
		void	Set_TRUNK_IN_TYPE_ID(long	lParam);
		void	Set_TRUNK_OUT(char	* sParam);
		void	Set_TRUNK_OUT_TYPE_ID(long	lParam);
...
inline ostream& operator<< (ostream& os, const TBL_CALL_EVENT_CLASS &obj) {
	obj.Print(os);
	return os;
}
...
class MEM_CALL_EVENT_CLASS{
	private:
		...
		static	vector<_TBL_CALL_EVENT_STR_>		a_item;
		static	hash_map<const long ,_TBL_CALL_EVENT_STR_* , hash<long>, MEM_CALL_EVENT_CLASS::eqlong>		hashmap;
	public:
		MEM_CALL_EVENT_CLASS(char * sTableName=NULL);
		~MEM_CALL_EVENT_CLASS();
		void	Load(char *sTableName=NULL);
		void	UnLoad(void);
		_TBL_CALL_EVENT_STR_ *	operator [](long opt);
};
