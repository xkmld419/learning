...
bool	TBL_BILL_CLASS::Next()
{
		...
		try{
			...
			item.BILL_AMOUNT=m_qry->field("BILL_AMOUNT").asFloat();
			item.LATE_FEE=m_qry->field("LATE_FEE").asFloat();
			item.DERATE_LATE_FEE=m_qry->field("DERATE_LATE_FEE").asFloat();
			item.BALANCE=m_qry->field("BALANCE").asFloat();
			item.LAST_CHANGE=m_qry->field("LAST_CHANGE").asFloat();
			item.CUR_CHANGE=m_qry->field("CUR_CHANGE").asFloat();
			strcpy(item.CREATED_DATE,m_qry->field("CREATED_DATE").asString());
			strcpy(item.PAYMENT_DATE,m_qry->field("PAYMENT_DATE").asString());
			item.USE_DERATE_BLANCE=m_qry->field("USE_DERATE_BLANCE").asFloat();
			item.INVOICE_ID=m_qry->field("INVOICE_ID").asFloat();
...
{
		if ( strlen(sParam)>14 ) 
			 throw TException( " Set_PAYMENT_DATE 字符参数的长度超过14!");
...
void	TBL_BILL_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
MEM_BILL_CLASS::MEM_BILL_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_BILL_CLASS::Load(char *sTableName=NULL)
{
