...
bool	TBL_BALANCE_PAYOUT_CLASS::Next()
{
		...
		try{
			if ( !m_qry->next())
				return false;
			item.OPER_PAYOUT_ID=m_qry->field("OPER_PAYOUT_ID").asFloat();
			...
			item.PRN_COUNT=m_qry->field("PRN_COUNT").asFloat();
...
void	TBL_BALANCE_PAYOUT_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
MEM_BALANCE_PAYOUT_CLASS::MEM_BALANCE_PAYOUT_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_BALANCE_PAYOUT_CLASS::Load(char *sTableName=NULL)
{
