...
bool	TBL_PAYMENT_CLASS::Next()
{
		...
		try{
			...
			item.PAYED_METHOD=m_qry->field("PAYED_METHOD").asFloat();
			strcpy(item.OPERATION_TYPE,m_qry->field("OPERATION_TYPE").asString());
			item.OPERATED_PAYMENT_SERIAL_NBR=m_qry->field("OPERATED_PAYMENT_SERIAL_NBR").asFloat();
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_OPERATION_TYPE 字符参数的长度超过3!");
...
void	TBL_PAYMENT_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
MEM_PAYMENT_CLASS::MEM_PAYMENT_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_PAYMENT_CLASS::Load(char *sTableName=NULL)
{
