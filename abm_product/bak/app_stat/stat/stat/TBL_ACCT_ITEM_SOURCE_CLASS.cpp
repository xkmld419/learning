...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_ITEM_SOURCE_TYPE 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>50 ) 
			 throw TException( " Set_NAME 字符参数的长度超过50!");
...
void	TBL_ACCT_ITEM_SOURCE_CLASS::Set_DESCRIPT(char	* sParam)
{
		if ( strlen(sParam)>250 ) 
			 throw TException( " Set_DESCRIPT 字符参数的长度超过250!");
...
void	TBL_ACCT_ITEM_SOURCE_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
MEM_ACCT_ITEM_SOURCE_CLASS::MEM_ACCT_ITEM_SOURCE_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_ACCT_ITEM_SOURCE_CLASS::Load(char *sTableName=NULL)
{
