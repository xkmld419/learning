...
bool	TBL_ACCT_BALANCE_CLASS::Next()
{
		...
		try{
			...
			item.BALANCE_TYPE_ID=m_qry->field("BALANCE_TYPE_ID").asFloat();
			...
			item.CYCLE_UPPER=m_qry->field("CYCLE_UPPER").asFloat();
			item.CYCLE_LOWER=m_qry->field("CYCLE_LOWER").asFloat();
			...
			item.ITEM_GROUP_ID=m_qry->field("ITEM_GROUP_ID").asFloat();
		}
		catch (TOCIException &oe) {
			cout<<"Error occured ... in TBL_ACCT_BALANCE_CLASS.cpp"<<endl;
			cout<<oe.getErrMsg()<<endl;
...
{
		if ( strlen(sParam)>14 ) 
			 throw TException( " Set_EFF_DATE 字符参数的长度超过14!");
...
{
		if ( strlen(sParam)>14 ) 
			 throw TException( " Set_EXP_DATE 字符参数的长度超过14!");
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_CYCLE_UPPER_TYPE 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_CYCLE_LOWER_TYPE 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>30 ) 
			 throw TException( " Set_BANK_ACCT 字符参数的长度超过30!");
...
void	TBL_ACCT_BALANCE_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
MEM_ACCT_BALANCE_CLASS::MEM_ACCT_BALANCE_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_ACCT_BALANCE_CLASS::Load(char *sTableName=NULL)
{
...
int main()
{
	gpDBLink.connect("acct","acct","jsbill");
	
	try{
		int i;
		TBL_ACCT_BALANCE_CLASS	a;
		char sTemp[256];
		for (i=1;i<70000;i=i+7){
			sprintf(sTemp,"ACCT_BALANCE_ID=%d",i);
			a.AddCondition(sTemp);
			a.SetTableName("ACCT.ACCT_BALANCE");
			a.Open();
			if (a.Next()){
				if (i%10==0)
					cout<<a<<endl;
