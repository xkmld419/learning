...
GetNextTableName_Lable2:		
		for (;;){
			if (!bOpenFlag){
				...
				sprintf(sSql,"SELECT TABLE_NAME FROM STAT_CLASS_DATA_SOURCE WHERE TABLE_NAME>'%s' AND STATE='F0A' AND STAT_CLASS_ID=%d ORDER BY TABLE_NAME",sTableName,iStatClassID);
				qry.close();
				qry.setSQL(sSql);
				qry.open();
...
GetNextTableName_Lable:		
		for (;;){
			if (!bOpenFlag){
				...
				sprintf(sSql,"SELECT TABLE_NAME,BREAKPOINT FROM STAT_CLASS_DATA_SOURCE WHERE TABLE_NAME>'%s' AND STATE='F0A' AND STAT_CLASS_ID=%d ORDER BY TABLE_NAME",sTableName,iStatClassID);
				qry.close();
				qry.setSQL(sSql);
				qry.open();
