...
bool	TBL_BALANCE_SOURCE_CLASS::Next()
{
		...
		try{
			if ( !m_qry->next())
				return false;
			item.PAYMENT_ID=m_qry->field("PAYMENT_ID").asFloat();
			item.BALANCE=m_qry->field("BALANCE").asFloat();
			item.OPER_INCOME_ID=m_qry->field("OPER_INCOME_ID").asFloat();
			item.ACCT_BALANCE_ID=m_qry->field("ACCT_BALANCE_ID").asFloat();
			...
			item.ACCT_BALANCE_OBJ_ID=m_qry->field("ACCT_BALANCE_OBJ_ID").asFloat();
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_OPER_TYPE 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>14 ) 
			 throw TException( " Set_OPER_DATE 字符参数的长度超过14!");
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_SOURCE_TYPE 字符参数的长度超过3!");
...
void	TBL_BALANCE_SOURCE_CLASS::Set_SOURCE_DESC(char	* sParam)
{
		if ( strlen(sParam)>250 ) 
			 throw TException( " Set_SOURCE_DESC 字符参数的长度超过250!");
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_ALLOW_DRAW 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>3 ) 
			 throw TException( " Set_CURR_STATE 字符参数的长度超过3!");
...
{
		if ( strlen(sParam)>14 ) 
			 throw TException( " Set_CURR_DATE 字符参数的长度超过14!");
...
void	TBL_BALANCE_SOURCE_CLASS::SetBuf(int iBufLen=ARRAY_BUFLEN)
{
		iBufferSize=iBufLen;
...
void	TBL_BALANCE_SOURCE_CLASS::SaveFlush(void)
{
		...
		ColumnList="PAYMENT_ID,BALANCE,OPER_INCOME_ID,ACCT_BALANCE_ID,OPER_TYPE,STAFF_ID,OPER_DATE,AMOUNT,SOURCE_TYPE,SOURCE_DESC,STATE,STATE_DATE,ACCT_BALANCE_OBJ_ID,INV_OFFER,ALLOW_DRAW,CURR_STATE,CURR_DATE";
		ParamList="";
...
MEM_BALANCE_SOURCE_CLASS::MEM_BALANCE_SOURCE_CLASS(char *sTableName=NULL)
{
		if (sTableName==NULL)
			Load();
		else
...
void	MEM_BALANCE_SOURCE_CLASS::Load(char *sTableName=NULL)
{
