/*VER: 1*/ 
#ifndef _TBL_DATA_EVENT_CLASS_H__
#define _TBL_DATA_EVENT_CLASS_H__
#include <iostream>
#include "TOCIQuery.h"
#include <string>
#include <vector>
...
using namespace std;
typedef struct _TBL_DATA_EVENT_STR_{
	long	EVENT_ID;
	long	BILL_FLOW_ID;
	...
	void	ClearBuf(void){
			memset(&EVENT_ID,0,sizeof(_TBL_DATA_EVENT_STR_));
	};
}_TBL_DATA_EVENT_STR_;

inline ostream& operator<< (ostream& os, const _TBL_DATA_EVENT_STR_ &obj) {
	obj.Print(os);
	return os;
}
...
class TBL_DATA_EVENT_CLASS{
	friend class MEM_DATA_EVENT_CLASS;
	private:
		_TBL_DATA_EVENT_STR_		item;
		_TBL_DATA_EVENT_STR_		*a_item;
		int		iCurPos;
		TOCIQuery	*		m_qry;
...
inline ostream& operator<< (ostream& os, const TBL_DATA_EVENT_CLASS &obj) {
	obj.Print(os);
	return os;
}
...
class MEM_DATA_EVENT_CLASS{
	private:
		...
		static	vector<_TBL_DATA_EVENT_STR_>		a_item;
		static	hash_map<const long ,_TBL_DATA_EVENT_STR_* , hash<long>, MEM_DATA_EVENT_CLASS::eqlong>		hashmap;
		static	bool	bLoadFlag;
	public:
		MEM_DATA_EVENT_CLASS(char * sTableName=NULL);
		~MEM_DATA_EVENT_CLASS();
		void	Load(char *sTableName=NULL);
		void	UnLoad(void);
		_TBL_DATA_EVENT_STR_ *	operator [](long opt);
};
