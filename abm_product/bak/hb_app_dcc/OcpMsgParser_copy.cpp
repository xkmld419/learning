...
void print(const AAAAvpContainer & c)
{
	for(int j = 0; j < c.size(); j++)	
	{
		...
		case AAA_AVP_ENUM_TYPE:
			{
				cout << ntohl(*ce->dataPtr(Type2Type<int>()));
				break;
			}
...
void OcpMsgParser::setDictHandle(bool output) {
  std::string serviceContextId;
    ...
    ::setDictHandle(*m_oMsg, "", serviceType, serviceIdentifier.c_str());
...
void OcpMsgParser::initLocalInfo(char *sOriginHost, char *sOriginRealm)
{
        ...
        case COMMAND_CODE_CC:
            {                
                ...
                if (m_oMsg->hdr.flags.r) {
                    ...
                    iType = CCA;
                    if(bIsActiveCC) {
                        getAvpValue("Service-Information.Service-Result-Code", uiTemp);
                        sprintf(buf, "%d", uiTemp);
                        strData = buf;
                    } else {
        ...
        case COMMAND_CODE_CE:
            {
                if (m_oMsg->hdr.flags.r) {
                    ...
                    sprintf((char *)strData.c_str() , "%d", uiTemp);
                }
                break;
            }
          default:
...
int OcpMsgParser::createCER(char *sArguments, char *sCERbuf, int iCERSize)
{
    ...
    setAvpValue("Origin-Host", m_strOriginHost); 
    setAvpValue("Origin-Realm", m_strOriginRealm); 
    setAvpValue("Host-IP-Address", m_localIp);
...
/*
生成激活CCR包
	int createActiveCCR( char *sArguments, char (*contextIDs)[256], int iRows, 
char *sCCRbuf, int iCCRSize);
	sArguments：传入参数 
		形式：Session-Id|hh|ee|Destination-Realm|Auth-Application-Id|Service-Context
-Id|CC-Request-Type|CC-Request-Number
...
*/
int OcpMsgParser::createActiveCCR(char *sArguments, char (*contextIDs)[256], 
int iRows, char *sCCRbuf, int iCCRSize)
{
    string str;
    unsigned int uiTemp;
    int iTemp;
...
*/
int OcpMsgParser::createServCCR(char *sSrcBuf, int iSrcSize, char *sCCRbuf, 
int iCCRSize)
{
    string strAvp;
...
*/
int OcpMsgParser::createServCCA(char *sSrcBuf, int iSrcSize, char *sCCAbuf, 
int CCASize)
{
    string strAvp;
...
/*
int parserPack(char *sMsgBuf, int iMsgSize, char *sParseBuf, int iParseSize, 
char *sServiceContextID);
	// 在此函数中解析消息后，判断包类型是CCR、CCA或激活CCA时，将解析内容放到
sParseBuf
...
int OcpMsgParser::parserPack(char *sMsgBuf, int iMsgSize, char *sParseBuf, int 
iParseSize, char *sServiceContextID)
{
    string strAvp;
    string strData;
    int ret;
