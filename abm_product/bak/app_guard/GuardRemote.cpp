...
int main(int argc, char ** argv)
{
    ...
    try {
        ...
        if (p) {
            strcpy (sFileName, p);
            strcat (sFileName,"/etc/tuxenv.ini");
        }
        else {
        ...
        int iret = gethostname(sHostName,100);
        if (iret) { //Can't get hostname
        ...
        iret = tuxreadenv (sFileName,"TUXCOMM");
        if (iret<0) {
...
int InitRmtShm (int iCrtFlg)
{
    ...
    g_lRmtWorkFlowKey = IpcKeyMgr::getIpcKey (-1,"SHM_WorkFlowKey");
