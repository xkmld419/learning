...
#define LOG_TO_THREE_GROUP
#ifdef LOG_TO_THREE_GROUP
#include "HSSLogV3.h"
...
int TRouteTable::startCommand(TServerAppInfo *pServApp)
{
    ...
    switch(iForkRet){
        ...
        case 0:
            ...
            if (execve(sCommand,argv,environ) <=0) {
              g_pLogg->log(MBC_THREAD_CREAT_FAIL,LOG_LEVEL_FATAL,LOG_TYPE_SYSTEM,"派生进程或线程失败[%s/%s],请检查程序是否存在",sPath,sExecName);
			}
