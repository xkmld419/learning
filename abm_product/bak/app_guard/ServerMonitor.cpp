...
#ifdef WF_DEBUG
    TWfLog::log("MONITOR_LOGIN ------- paraStr=%s; StrLen=%d",sQqstBuf,iLen);
#endif
...
#ifdef WF_DEBUG
    TWfLog::log("MONITOR_LOGIN-- return_Str=%s",sQqstBuf);
#endif
...
#ifdef WF_DEBUG
    TWfLog::log("RECV_APPINFO ------ paraStr=%s",sQqstBuf);
#endif
...
#ifdef WF_DEBUG
    TWfLog::log("RECV_APPINFO: return_str=%s \n for AuthID:%d",sQqstBuf,g_pShmTable->MntReqInfo.m_iAuthID );
#endif
...
/***********************************************************************
    ...
    输入参数:
        iLogID(登录ID)
        iAuthID(身份认证ID)
        iProcessID(对应WF_PROCESS.PROCESS_ID)
...
#ifdef WF_DEBUG
    TWfLog::log("SEND_COMMAND ------- paraStr=%s",sQqstBuf);
#endif
...
/*        sprintf (sTemp," 远程守护进程GuardRemote尚未启动!\n"
	...
	sprintf(sTemp,"所执行的进程非本主机本用户下的进程,\n"
        	"Action:请选择正确的用户登录!");
...
#ifdef WF_DEBUG
    TWfLog::log("SEND_COMMAND --- ret_Str=%s",sQqstBuf);
#endif
...
#ifdef WF_DEBUG
    TWfLog::log("CHANGE_STATUS ------- paraStr=%s",sQqstBuf);
#endif
...
#ifdef WF_DEBUG
    TWfLog::log("REFRESH_RUNPLAN: return_str=%s \n",sQqstBuf);
#endif
