...
#include "AsigServiceSoapBinding.nsmap"
    ...
    { 
      ...
      { 
        ...
        { SOAP_SOCKLEN_T k = (SOAP_SOCKLEN_T)sizeof(soap->peer);
          ...
          r = recvfrom(soap->socket, s, (SOAP_WINSOCKINT)n, soap->socket_flags, (struct sockaddr*)&soap->peer, (int *)&k);	/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
  ...
  { 
    ...
    { err = soap_socket_errno(fd);
	  ...
	  r = soap->errnum;
        ...
        if (!getsockopt(fd, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum, (int *)&k) && !soap->errnum)	/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
          break;
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not connect to host\n"));
        if (!soap->errnum)
          soap->errnum = soap_socket_errno(fd);
...
{ SOAP_SOCKET fd;
  fd = accept(s, a, (int*)(SOAP_SOCKLEN_T*)n); /* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
#ifdef SOCKET_CLOSE_ON_EXEC
#ifdef WIN32
#ifndef UNDER_CE
  SetHandleInformation((HANDLE)fd, HANDLE_FLAG_INHERIT, 0);
