    ...
    SHMAccess (shmname, extSize, iMaxExtCnt )
{
    ...
    if (m_poSHM){
        ...
        if( SHM_LRU_CURR_VERSION != *puiVer ){
            ALARMLOG28(0,MBC_CLASS_Fatal, 4,"%s:%s","共享内存连接失败",shmname);
    ...
    SHMAccess (shmkey, extSize, iMaxExtCnt )
{
    ...
    if (m_poSHM){
        open ();
        if( SHM_LRU_CURR_VERSION != *puiVer ){
            ALARMLOG28(0,MBC_CLASS_Fatal, 4,"%s:%ld","共享内存连接失败",shmkey);
            THROW(MBC_SHMLruData+1);
        }
...
try{
    if(0 == (*puiDelete) ){
        ...
        *puiDelete = m_pPointer[iReal].uiListIdxNext;
        if(iTemp == (*puiDelete) ){
            ////最后一个删除的项被分配出来
            (*puiDelete) =0;
        }
...
try{
    ...
    m_oDataLock.V();
    ALARMLOG28(0,MBC_CLASS_Memory, 120,"%s:%u","删除共享内存失败",indexBig);
    throw;
}
    m_oDataLock.V();
    ALARMLOG28(0,MBC_CLASS_Memory, 120,"%s:%u","删除共享内存失败",indexBig);
...
void SHMLruData<T>::open()
{
    ...
    ////后面从puiVer+ 32 开始到puiVer+53 预留以后扩展使用。
    ...
    if( !bForce ){
        ...
        switch( *puiState){
            case 1:
            {
            	...
            	struct timezone tz;
                if(0 == gettimeofday (&tv , &tz) ){
                    if(tv.tv_sec >= lClock + SHM_LRU_DIRTY_CLOCKCNT ){
                        ///时间到
                        lClock = tv.tv_sec;
                        printf("到达定时阀值\n");
                        break;
...
//                printf("2in file at %u\n", uiFile);
            ...
            if( uitemp ){
                ...
                if( SHM_LRU_FLAG_DEL ==  m_pPointer[ uitemp ].sFlag[0] ){
                    ////不应该走到这里
                    printf("write deleted data confused..\n");
                }else{
                    poFileData[uiFile].uiNext = 0;
    ...
    for(int i=0; i< SHM_LRU_MAX_SESSION; i++){
        ...
        while( *pDirty ){
            ...
            if( uitemp ){
                ...
                if( SHM_LRU_FLAG_DEL == m_pPointer[ *pDirty ].sFlag[0] ){
                    delFromIndex(m_pPointer[ *pDirty ].uiListIdxID, 1);
                    ///delFromIndex改写了uiListIdxNext和uiListIdxID和sFlag和(*puiDelete)
