...
bool AcctItemMgr::getBelongGroup(int acctitem, int acctitemgroup)
{
    ...
    if(!(G_PPARAMINFO->m_poAcctItemGroupIndex->get(acctitemgroup, &iIdx)))
    {
        return false;
    }
    else
    ...
    if(G_PPARAMINFO->m_poAcctItemIncludeDisctIndex->get(bacctitemgroup, &iDiscount))
    {
        switch(iDiscount)
        {
            case 0:
            {
                ...
                {
                    if(!(G_PPARAMINFO->m_poAcctItemGroupBIndex->get(bacctitemgroup, &iIdx)))
                    {
                        return false;
                    }
                    else
...
bool AcctItemMgr::getBelongGroupB(int acctitem, int bacctitemgroup, ParamInfoMgr* pParamInfoMgr)
{
    unsigned int iIdx = 0;
    ItemGroupMember *pItemMember = 0;
    if(!(pParamInfoMgr->m_poAcctItemGroupBIndex->get(bacctitemgroup, &iIdx)))
    {
        return false;
    }
    else
