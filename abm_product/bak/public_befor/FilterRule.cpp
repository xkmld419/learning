...
bool FilterRuleCheckValue::checkSwitchIDIndex(StdEvent * pEvent)
{
	 ...
	 {	//看是否有默认值 没有就返回失败//m_iSwitchID == -1
		if(bDefault){
			...
			sKey[0] = '\0';
			sprintf(sKey,"%d_%d",pEvent->m_oEventExt.m_iSwitchLongType,-1);
			if (!G_PPARAMINFO->m_poRuleCheckSwitchIDIndex->get (sKey, &iRuleRes)) 
	 		{			
				return false;
			} else {
...
bool FilterRuleCheckValue::checkTrunkInIndex(StdEvent * pEvent)
{
	 ...
	 if(pEvent->m_sTrunkIn[0] != '*'){
	 	sprintf(sKey,"%d_%s",pEvent->m_iSwitchID,pEvent->m_sTrunkIn);
	 } else {
	 	sprintf(sKey,"%d_%s",pEvent->m_iSwitchID,"*");
		bDefault = true;
	 }
	 unsigned int iRuleRes = 0;
...
bool FilterRuleCheckValue::checkTrunkOutIndex(StdEvent * pEvent)
{
	 ...
	 if(pEvent->m_sTrunkOut[0] != '*'){
	 	sprintf(sKey,"%s_%s",pEvent->m_sTrunkIn,pEvent->m_sTrunkOut);
	 } else {
	 	sprintf(sKey,"%s_%s",pEvent->m_sTrunkIn,"*");
		bDefault = true;
	 }
	 unsigned int iRuleRes = 0;
...
bool FilterRuleCheckValue::checkGatherPotIndex(StdEvent * pEvent)
{
	 ...
	 sprintf(sKey,"%s_%d",pEvent->m_sTrunkOut,m_iPot);
	 if(m_iPot == -1)
	 	bDefault = true;
	 ...
	 {	//看是否有默认值 没有就返回失败
	 	if(bDefault) {
			...
			sKey[0] = '\0';
			sprintf(sKey,"%s_%d",pEvent->m_sTrunkOut,-1);
			if (!G_PPARAMINFO->m_poRuleCheckGatherPotIndex->get (sKey, &iRuleRes)) 
	 		{			
				return false;
			} else {
...
bool FilterRuleCheckValue::checkCallingIndex(StdEvent * pEvent)
{
	 ...
	 if(pEvent->m_sOrgCallingNBR[0] != '*'){
	 	sprintf(sKey,"%s_%s",pEvent->m_oEventExt.m_sSwitchAreaCode,pEvent->m_sOrgCallingNBR);
	 } else {
	 	sprintf(sKey,"%s_%s",pEvent->m_oEventExt.m_sSwitchAreaCode,"*");
		bDefault = true;
	 }	
	 unsigned int iRuleRes = 0;
...
bool FilterRuleCheckValue::checkCalledIndex(StdEvent * pEvent)
{
	 ...
	 if(pEvent->m_sOrgCalledNBR[0] != '*'){
	 	sprintf(sKey,"%s_%s",pEvent->m_sOrgCallingNBR,pEvent->m_sOrgCalledNBR);
	 } else {
	 	sprintf(sKey,"%s_%s",pEvent->m_sOrgCallingNBR,"*");
		bDefault = true;
	 }	
	 unsigned int iRuleRes = 0;
