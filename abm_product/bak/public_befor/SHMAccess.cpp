...
SHMAccess::SHMAccess(char *shmname) :
...
void SHMAccess::open()
{
	unsigned long size; 
	if (m_sSHMName) {
		m_poSHM = new SimpleSHM (m_sSHMName, 4);
	} else {
...
#ifdef SHM_EXPAND_FREE_POS		
		if (m_sSHMName) {
			m_poSHM = new SimpleSHM (m_sSHMName, size, g_sGlobalPos);
		} else {
...
#else
		if (m_sSHMName) {
			m_poSHM = new SimpleSHM (m_sSHMName, size, 0);
		} else {
		    ////printf("key:0x%lx need addr-->%lx\n", m_lSHMKey, 0);
...
bool SHMAccess::openEx(int i)
{
	unsigned long size; 
	if (m_sExSHMName[i]) {
		m_poExSHM[i] = new SimpleSHM (m_sExSHMName[i], 4);
	} else {
	...
	if (m_poExSHM[i]->exist()) {
		...
		if (m_sExSHMName[i]) {
			m_poExSHM[i] = new SimpleSHM (m_sExSHMName[i], size,sExBase );
		} else {
			m_poExSHM[i] = new SimpleSHM (m_lExSHMKey[i], size, sExBase );
		}
#else
		if (m_sExSHMName[i]) {
			m_poExSHM[i] = new SimpleSHM (m_sExSHMName[i], size,0 );
		} else {
...
{ 
	...
	if (m_sExSHMName[m_iExShmCnt]) {
		m_poExSHM[m_iExShmCnt] = new SimpleSHM (m_sExSHMName[m_iExShmCnt], m_lNextSize, sExBase  );
	} else {
	...
	if (m_sExSHMName[m_iExShmCnt]) {
		m_poExSHM[m_iExShmCnt] = new SimpleSHM (m_sExSHMName[m_iExShmCnt], m_lNextSize, 0 );
	} else {
