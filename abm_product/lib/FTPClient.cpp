...
//处理ftp时间格式, 输出标准时间格式 
// 支持格式1: "Aug  7  2007"  -- 'Mon DD  YYYY'
// 支持格式2: "Mar  5 19:02"  -- 'Mon DD HH24:MI'
bool CFTPClient::changeDateFormat (char *sTempDate, char *sOutDate)
{
    char sMon[4], sDate[3], sTime[6], sYear[5];
    
    int iMon;
    
    Date dNow;
    
    if (strlen (sTempDate) == 12) 
    {
        //取月份
        strncpy (sMon, sTempDate, 3);
        sMon[3] = 0;
        
        if      (!strcasecmp(sMon,"Jan"))  iMon = 1;
        else if (!strcasecmp(sMon,"Feb"))  iMon = 2;
        else if (!strcasecmp(sMon,"Mar"))  iMon = 3;
        else if (!strcasecmp(sMon,"Apr"))  iMon = 4;
        else if (!strcasecmp(sMon,"May"))  iMon = 5;
        else if (!strcasecmp(sMon,"Jun"))  iMon = 6;
        else if (!strcasecmp(sMon,"Jul"))  iMon = 7;
        else if (!strcasecmp(sMon,"Aug"))  iMon = 8;
        else if (!strcasecmp(sMon,"Sep"))  iMon = 9;
        else if (!strcasecmp(sMon,"Oct"))  iMon = 10;
        else if (!strcasecmp(sMon,"Nov"))  iMon = 11;
        else if (!strcasecmp(sMon,"Dec"))  iMon = 12;
        else {
            return false;
        }
        
        //取日期
        strncpy (sDate, sTempDate+4, 2);
        sDate[2] = 0;
        int iDate = atoi(sDate);
        if (iDate<1 || iDate>31)
            return false;
        sprintf (sDate,"%02d",iDate);
        
        //确定年份,时间
        strcpy (sTime, sTempDate+7);        
        if (sTime[0] == ' ' && sTime[2]!=':') {
                                //最后是年份 格式1  "Aug  7  2007"  -- 'Mon DD  YYYY'
            strcpy (sYear, sTime+1);
            strcpy (sTime, "0000"); //默认0时0分
        }
        else if (sTime[2] == ':') {
                                           //格式2  "Mar  5 19:02"  -- 'Mon DD HH24:MI'
                                           //       "Mar  5  0:02"  -- 'Mon DD HH24:MI'
            strncpy (sYear, dNow.toString(), 4);
            sYear[4] = 0;
    ...
    sprintf (sOutDate,"%s%02d%s%s00",sYear,iMon,sDate,sTime);
    
    Date dRet (sOutDate);    
    
    if (dNow.diffMon (dRet) > 3) {// 取得的时间比当前系统时间还大,异常
        dRet.addYear(-1);
        strcpy (sOutDate, dRet.toString());
    }
