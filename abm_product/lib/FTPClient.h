...
public: 
  ...
  int FTPLogin(const char *strServer, const char *strUser,
      const char *strPassword=NULL, const int i_iPort=21);  

  int FTPCmd(FTPCMDTYPE cmd, const char* cmdvalue=NULL, char *returnvalue=NULL, int retmaxsize = 0);

  int GetFile(const char* strRemote, const char* strLocal = NULL);
...
private:
 ...
 public:
    static bool changeDateFormat(char *sTempDate, char *sDate);
