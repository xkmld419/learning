 ...
 * separate concerns.
 */
class  ACE : public ACE_Flag_Manip, public ACE_Handle_Ops,
                       public ACE_Init_ACE,
...
// Wrappers for methods that have been moved to ACE_OS.
  ...
  inline static ssize_t send_n (ACE_HANDLE handle,
             const void *buf,
             size_t len,
             int flags,
   ...
   * a new char  array is allocated and a copy of @c pathname with
  ...
  static const char  *dirname (const char  *pathname,
                                   char  delim = ACE_DIRECTORY_SEPARATOR_CHAR);
   ...
   * zombies.
   */
  static pid_t fork (const char  *program_name = ACE_LIB_TEXT ("<unknown>"),
                     int avoid_zombies = 0);
   ...
   * closed.
   */
  static int daemonize (const char  pathname[] = ACE_LIB_TEXT ("/"),
                        int close_all_handles = ACE_DEFAULT_CLOSE_ALL_HANDLES,
                        const char  program_name[] = ACE_LIB_TEXT ("<unknown>"));
  ...
  static inline u_long log5 (u_long num)
  {
    u_long log = 0;
