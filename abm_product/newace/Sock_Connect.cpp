// Sock_Connect.cpp,v 1.34 2004/01/29 20:07:41 bala Exp
                     ...
                     ACE_HANDLE handle)
{
  ...
  int cmd = SIOCGIFCONF;
#endif /* AIX */
  if (ioctl (s, cmd, (char *) &ifc) == -1)
    ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_LIB_TEXT ("%p\n"),
                       ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr:ioctl (get interface configuration)")),
                      -1);
  ...
  if (host_name)
    {
...
#else /* _UNICOS */
      {
        ...
        memcpy(haddrp,(char *) hp->h_addr,hp->h_length);
  ...
  for (int n = ifc.ifc_len / sizeof (struct ifreq);
...
#endif /* !CHORUS_4 && !__FreeBSD__ */
    {
      ...
      if (ifr->ifr_addr.sa_family != AF_INET)
        {
          ACE_ERROR ((LM_ERROR,
                      ACE_LIB_TEXT ("%p\n"),
                      ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr:Not AF_INET")));
          continue;
        }
      ...
      if (ioctl (s, SIOCGIFFLAGS, (char *) &flags) == -1)
        {
          ACE_ERROR ((LM_ERROR,
                      ACE_LIB_TEXT ("%p\n"),
                      ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr: ioctl (get interface flags)")));
          continue;
        }
      ...
      if (ACE_BIT_DISABLED (flags.ifr_flags, IFF_UP))
        {
          ACE_ERROR ((LM_ERROR,
                      ACE_LIB_TEXT ("%p\n"),
                      ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr:Network interface is not up")));
          continue;
        }
      ...
      if (ACE_BIT_ENABLED (flags.ifr_flags, IFF_BROADCAST))
        {
          if (ioctl (s,
            ...
            ACE_ERROR ((LM_ERROR,
                        ACE_LIB_TEXT ("%p\n"),
                        ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr:ioctl (get broadaddr)")));
          else
            {
              memcpy (ACE_reinterpret_cast(sockaddr_in *, &ip_addr),
        ...
        ACE_ERROR ((LM_ERROR,
                    ACE_LIB_TEXT ("%p\n"),
                    ACE_LIB_TEXT ("ACE_Sock_Connect::get_bcast_addr:Broadcast is not enable for this interface.")));
  ...
  //       library for WinCE.
    ...
    while (result != ERROR_SUCCESS)
    {
        ...
        case ERROR_BUFFER_OVERFLOW:  // MUST come here at the first run because sz = 0
            ...
            if (result == ERROR_SUCCESS) {
                ...
                // find out how many interfaces are there
                {
                    ...
                    if (n_interfaces == 0) {
                        ACE_ERROR_RETURN ((LM_ERROR,
                            ACE_LIB_TEXT ("%p\nACE_Sock_Connect::get_ip_interfaces - No adapter found.")),
                            -1);
                    }
        ...
        case ERROR_NOT_SUPPORTED: // OS does not support this method
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_LIB_TEXT ("%p\nACE_Sock_Connect::get_ip_interfaces - This version of WinCE does not support GetAdapterInfo.")),
                -1);
            break;

        case ERROR_NO_DATA:  // no adapter installed
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_LIB_TEXT ("%p\nACE_Sock_Connect::get_ip_interfaces - No network adapter installed.")),
                -1);
            break;

        case ERROR_INVALID_PARAMETER:
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_LIB_TEXT ("%p\nACE_Sock_Connect::get_ip_interfaces - Invalid parameter.")),
                -1);
            break;

        default:
            ACE_ERROR_RETURN ((LM_ERROR,
                ACE_LIB_TEXT ("%p\nACE_Sock_Connect::get_ip_interfaces - Adapter info access permission denied.")),
                -1);
            break;
        }
    }
...
# endif /* Winsock 2 && MSVC 5 or later */
  ...
  ACE_NEW_RETURN (ifs,
                  ...
                  -1);
  memset (ifs, 0, num_ifs * sizeof (struct ifreq));
  ...
  struct ifconf ifcfg;
  memset (&ifcfg, 0, sizeof (struct ifconf));
  ...
  if (ioctl (handle,
                     ...
                     (caddr_t) &ifcfg) == -1)
    {
      ...
      ACE_ERROR_RETURN ((LM_ERROR,
                         ACE_LIB_TEXT ("%p\n"),
                         "ACE_Sock_Connect::get_ip_interfaces:ioctl - SIOCGIFCONF failed"),
                        -1);
    }
...
ACE_Sock_Connect::count_interfaces (ACE_HANDLE handle, size_t &how_many)
{
  ...
  if (ioctl (handle,
    ...
    ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_LIB_TEXT ("%p\n"),
                       ACE_LIB_TEXT ("ACE_Sock_Connect::count_interfaces:ioctl - SIOCGIFNUM failed")),
                      -1);
  ...
  struct ifreq *p_ifs =
    (struct ifreq *) malloc (ifreq_size);
  ...
  if (ioctl (handle,
                     ...
                     (caddr_t) &ifcfg) == -1)
    {
      ...
      ACE_ERROR_RETURN ((LM_ERROR,
                         ACE_LIB_TEXT ("%p\n"),
                         ACE_LIB_TEXT ("ACE_Sock_Connect::count_interfaces:ioctl - SIOCGIFCONF failed")),
                        -1);
    }
...
#if defined (sparc) && ! defined (CHORUS) && !defined(DEF_SOLARIS)
  handle = ACE_OS::open ("/dev/udp", O_RDONLY);
#elif defined (__unix) || defined (__unix__) || defined (__Lynx__) || defined (_AIX) || defined(DEF_SOLARIS)
  // Note: DEC CXX doesn't define "unix" BSD compatible OS: HP UX,
  // AIX, SunOS 4.x
