// INET_Addr.cpp,v 4.125 2006/02/10 09:59:48 jwillemsen Exp
                               ...
                               int ipaddr_format) const
{
  ...
  size_t const total_len =
    (ipaddr_format == 0
     ? strlen (this->get_host_name ())
     : strlen (this->get_host_addr ()))
    + strlen ("65536") // Assume the max port number.
    + sizeof (':')
  ...
  if (port_p == 0) // Assume it's a port number.
    {
      char *endp = 0;
      u_short port =
        static_cast<u_short> (strtol (ip_addr, &endp, 10));
  ...
  else
    {
      *port_p = '\0'; ++port_p; // skip over ':'
      ...
      char *endp = 0;
      u_short port = static_cast<u_short> (strtol (port_p, &endp, 10));
      if (*endp == '\0')    // strtol scanned the entire string - all digits
...
#  if defined (ACE_HAS_IPV6)
  if (strcmp (ACE_TEXT_CHAR_TO_TCHAR(protocol), ACE_LIB_TEXT ("tcp6")) == 0)
...
ACE_INET_Addr::get_host_name_i (char hostname[], size_t len) const
{
...
#if defined (ACE_HAS_IPV6)
  if ((this->get_type () == PF_INET6 &&
       0 == memcmp (&this->inet_addr_.in6_.sin6_addr,
                            &in6addr_any,
  ...
  if (this->inet_addr_.in4_.sin_addr.s_addr == INADDR_ANY)
    ...
    {
      hostent *hp = gethostbyaddr ((char *)this->ip_addr_pointer (),
                                           this->ip_addr_size (),
                                           this->get_type ());
                                ...
                                int map /* = 0 */)
{
  ...
  if (len == 4)
  ...
  else if (len == 16)
    {
      ...
      this->base_set (AF_INET6, sizeof (this->inet_addr_.in6_));
      this->inet_addr_.in6_.sin6_family = AF_INET6;
      memcpy (&this->inet_addr_.in6_.sin6_addr, ip_addr, len);
...
#  if defined (ACE_VXWORKS)
  ...
  return &buf_[0];
#  else /* ACE_VXWORKS */
  return ::inet_ntoa (this->inet_addr_.in4_.sin_addr);
#  endif /* !ACE_VXWORKS */
#endif /* !ACE_HAS_IPV6 */
}
