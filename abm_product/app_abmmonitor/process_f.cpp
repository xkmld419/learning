...
int SearchNextNode (int *pLastBillFlowID, int *pLastProcID, int *pLastAppID)
{
    ...
    int iTmpAppID=*pLastAppID;
    int iSelecetdPos = -1;
    
    for (int i=0; i<pInfoHead->iProcNum; i++) {
        if((pProcInfoHead+i)->iAppID < iTmpAppID)
            continue;
        if ((pProcInfoHead+i)->iProcessID == -1) //节点已空闲
            continue;
        if(((pProcInfoHead+i)->iAppID == iTmpAppID)&&((pProcInfoHead+i)->iProcessID <=iTmpProcessID))
            continue;
        ...
        if ((pProcInfoHead+i)->iAppID > (pProcInfoHead+iSelecetdPos)->iAppID)
            continue;
        if (((pProcInfoHead+i)->iAppID== (pProcInfoHead+iSelecetdPos)->iAppID)
           &&((pProcInfoHead+i)->iProcessID >=(pProcInfoHead+iSelecetdPos)->iProcessID))
            continue;
