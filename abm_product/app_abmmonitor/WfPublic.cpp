...
{  
   ...
   else if (strcmp(pAtt,"iMqId") == 0) { 
	this->m_iMqId = atoi(pValue); 
    	}       else if (strcmp(pAtt,"iFlag") == 0) { 	this->m_iFlag = atoi(pValue);       	} 
    else {  
        //do nothing.  
    }      
}  
...
{  
    ...
    if (m_iMqId != 0) 
        sprintf(m_Str,"%s<iMqId=%d>",m_Str,m_iMqId);         if (m_iFlag !=0) 	    sprintf(m_Str,"%s<iFlag=%d>",m_Str,m_iFlag); 
    for (int iPos = 0; iPos<m_iMsgOff; iPos++) {  
...
int TSHMCMA::changeState(int _iProcID, char _cState) {  
    ...
    if (!pt_app) {  
        TWfLog::log(WF_FILE,"TSHMCMA::changeState, g_pShmTable.TAppInfo[?]中未找到 ProcessID = %d 对应的进程实例!",_iProcID);  
        return 0;  
    }  
...
{  
    ...
    char sql[1000];  
    if (_iAppPid == 0) {  
        sprintf( sql," update m_wf_log_process set state = '%c',end_date = sysdate "  
            " where process_log_id = %d ", _cState, _iProcessLogID);  
    }  
    else {  
        sprintf( sql," update m_wf_log_process set state = '%c',end_date = sysdate, app_pid = %d "  
...
{  
    ...
    char sql[1000];  
    sprintf( sql,"insert into m_wf_log_process( "  
...
//    DEFINE_QUERY(qry);           
    ...
    qry.close();  
    qry.setSQL("select seq_m_process_log_id.nextval log_id from dual");  
    qry.open();           
    qry.next();           
