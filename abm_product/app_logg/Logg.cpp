...
void LoggMgr::printUsage()
{
    ...
    cout << "*********************************************************************************" << endl;
    cout << "                                  展现类命令:                                    " <<endl;  
    cout << "abmlog" << " "  << "-param   [parameter] :                    显示[指定参数名]的日志内容"<<endl;
    cout << "abmlog" << " "  << "-process [process] :                      显示所有[指定进程ID]的日志内容"<<endl;
    //cout << "abmlog" << " "  << "-module  [module] :                       显示所有[指定模块ID]的日志内容"<<endl;
   // cout << "abmlog" << " "  << "-class   [class] :                        显示所有[指定类别ID]的日志内容"<<endl;
    cout << "abmlog" << " "  << "-level   [level] :                        显示所有[指定等级ID]的日志内容"<<endl;
//    cout << "abmlog" << " "  << "-type    [type] :                         显示所有[指定分类ID]的日志内容"<<endl;
    cout << endl;
    cout << "                                  控制类命令:                                    " <<endl;
    cout << "abmlog" << " "  << "-c :                                      创建日志组共享内存" << endl;
    cout << "abmlog" << " "  << "-f :                                      释放日志组共享内存" << endl;
    cout << "abmlog" << " "  << "-a :                                      日志组手动归档" << endl;
    //cout << "abmlog" << " "  << "-b groupid path(不指定备份到默认目录) :   备份指定日志组到指定目录" << endl;
   // cout << "abmlog" << " "  << "-d groupid filename :                     删除日志组文件" << endl;
    //cout << "abmlog" << " "  << "-e param :                                设置日志组归档" << endl;
    //cout << "abmlog" << " "  << "-i groupid path(不配取默认路径) :         增加日志组文件" << endl;
//    cout << "abmlogg" << " "  << "-g :                                      增加日志组" << endl;
    cout << "abmlog" << " "  << "-l (groupid)(带上groupid打印指定组) :     显示(当前/指定)日志组信息" << endl;
    cout << "abmlog" << " "  << "-s groupid(不指定自动切换当前组) :        切换日志组" << endl;
    cout << "*********************************************************************************" << endl;
    cout << endl << endl;
    return ;
}
        ...
        if(strlen(m_sParam))
        {
            ...
            if(iCondId==LOG_TYPE_BUSI)
              ...
              m_poLogg->GetInfoFromMem(DisplayLogg::m_vTypeOracle,iMode,LOG_TYPE_DATEDB,m_sParam);                	 
            	...
            	{
            	 printf("错误的日志类别!\n 日志类别取值如下：\n");
            	 //printf("1:BUSI  2:PARAM  3:SYSTEM  4:PROC  5:DB \n");
            	 printf("    1:业务日志  2:参数管理日志  3:系统管理日志  4:进程调度日志  5:操作类日志 \n");
               return false;   
              }     	     	
        }else{
           m_poLogg->GetInfoFromMem(DisplayLogg::m_vTypeBusi,iMode,LOG_TYPE_BUSI,m_sParam);  
    ...
    switch(m_iMode)
    {
        ...
        case GROUPQUIT:
            {
                ...
                if(m_poLogg->ChangeLogGroup(false,m_iGroupId)==true)
                {
					...
					Log::log(0,"%s",stemp);
					//m_poLogg->log(MBC_THREELOG_CHANGE,LOG_LEVEL_INFO,LOG_TYPE_SYSTEM,"abmlog -s 切换日志组成功:%s",stemp);
                }
                else
                {
                    Log::log(0,"切换日志组失败");
				//	m_poLogg->log(MBC_THREELOG_CHANGE,LOG_LEVEL_WARNING,LOG_TYPE_SYSTEM,"abmlog -s 切换日志组失败");
                    return false;
                }
        ...
        case PIGEONHOLE:
            {
                ...
                {
                    Log::log(0,"切换日志组失败");
					m_poLogg->log(MBC_THREELOG_PIGE,LOG_LEVEL_WARNING,LOG_TYPE_SYSTEM,"abmlog -a 手动归档日志组失败");
                    return false;
                }
                else
                {
                	m_poLogg->log(MBC_THREELOG_PIGE,LOG_LEVEL_INFO,LOG_TYPE_SYSTEM,"abmlog -a 手动归档日志组成功");
                    Log::log(0,"手动归档成功");
                }
            }
                //pPoint->touchInfoDown();
        ...
        case GROUPMODULE:
                ...
                { 
                    ...
                    {
                        printf("输入的ModuleID【%s】不存在,请通过sgwstat -ses查询模块信息\n",m_sParam);
                        return false;
                    }
                }
                DisplayInfo(m_iMode);
...
int LoggMgr::run()
{
    ...
    if(!init())
    {
        Log::log(0,"abmlog初始化失败");
        return -1;
    }
    if(!deal())
    {
        Log::log(0,"abmlog执行失败");
        return -3;
    }
    return 0;
}
