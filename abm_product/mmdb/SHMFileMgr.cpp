...
char sSHMFieldData[SHM_FIELD_NUM][SHM_FIELD_LEN];

SHMFileMgr::SHMFileMgr(int iMode){
    ...
    case TABLE_MIN_INFO:
        {
            ...
            writeFieldBuf("MIN_INFO_ID",FILEMGR_TRANS_LONG,
    ...
    while ( (sData[nCurrentPos]!=0) && (j<m_iFileFieldCnt) ) {
        if (sData[nCurrentPos] == m_sDivChar) {
            if ( (sData[nPrePos] == m_sDivChar) && (nPrePos!=0) && (nCurrentPos == nPrePos+1) ) {
                ...
                if (nPrePos !=0) {
                    strncpy(sSHMFieldData[j],&sData[nPrePos+1],nCurrentPos-nPrePos-1);
                    sSHMFieldData[j][nCurrentPos-nPrePos-1]=0;
                } else {
                    strncpy(sSHMFieldData[j],&sData[nPrePos],nCurrentPos-nPrePos);
                    sSHMFieldData[j][nCurrentPos-nPrePos]=0;
                }
            }
            nPrePos = nCurrentPos;
            j++;
...
bool FileMgr::writePair(void **sDes,const char* sSrc, const char cTransType, 
    ...
    switch (cTransType) {
    case SHM_FILEMGR_TRANS_NULL:
        {
            Log::log(0,"转换无法识别字段类型");
            return false;
        }
        break;
    case SHM_FILEMGR_TRANS_CHAR:
        {
            *(int*)((char*)pData+iOffset)= (int)sSrc[0];
        }
        break;
    case SHM_FILEMGR_TRANS_INT:
        {
            *(int*)((char*)pData+iOffset) = atoi(sSrc);
        }
        break;
    case SHM_FILEMGR_TRANS_LONG:
        {
            *(long*)((char*)pData+iOffset) = atol(sSrc);
        }
        break;
    case SHM_FILEMGR_TRANS_FLOAT:
        {
            *(float*)((char*)pData+iOffset) = atof(sSrc);   
        }
        break;
    case SHM_FILEMGR_TRANS_DOUBLE:
        {
            *(double*)((char*)pData+iOffset) = atof(sSrc);                   
        }
        break;
    case SHM_FILEMGR_TRANS_STRING:
    case SHM_FILEMGR_TRANS_EVTDATE:
        {
            if (iFieldLen>2) {
