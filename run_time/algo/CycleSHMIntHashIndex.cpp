...
#include "CycleSHMIntHashIndex.h"
#include "MBC.h"
...
CycleSHMIntHashIndex::CycleSHMIntHashIndex(ABMException &oExp,long shmkey) :
SHMAccess(oExp, shmkey), m_piHashValue(0), m_piTotal(0), m_piUsed(0), m_poHash(0)
{
    if (m_poSHM&&!getError()) {
		open ();
    }
...
CycleSHMIntHashIndex::~CycleSHMIntHashIndex()
{
}

long CycleSHMIntHashIndex::getValue(unsigned int key)
{
...
void CycleSHMIntHashIndex::add(long key, unsigned int value)
{
...
bool CycleSHMIntHashIndex::get(long key, unsigned int *pvalue)
{
	unsigned int i = (key & (*m_piHashValue));
	unsigned int j;
...
void CycleSHMIntHashIndex::reset()
{
	*m_piUsed = 0;
	*m_piHead = 1;
	*m_piTail = *m_piTotal;
    memset(m_poHash,0, (*m_piHashValue+1)*sizeof(unsigned int));
    memset (m_poList, 0, sizeof (CycleSHMIntList)*(*m_piTotal+1));

    for (int i=*m_piHead; i<*m_piTail; i++) {
        m_poList[i].iIdleNext = i+1;
    }
    m_poList[*m_piTail].iIdleNext = 0;    
}

//##ModelId=42442B02030B
void CycleSHMIntHashIndex::create(ABMException &oExp,
    unsigned int itemnumber, unsigned int iPermission) 
{
	//##计算HASHVALUE
	...
	unsigned long size = (hashvalue+1)*sizeof(unsigned int) + \
				(itemnumber+1)*sizeof(CycleSHMIntList) + \
				6*sizeof(unsigned int);
	SHMAccess::create (oExp, size, iPermission);
	...
	*m_piTail  = *m_piTotal;
	...
	m_poList = (CycleSHMIntList *)(m_poHash + (*m_piHashValue)+1);
...
void CycleSHMIntHashIndex::open()
{
	m_piHashValue = (unsigned int *)((char *)(m_poSHM->getPointer())) + 1;
...
void CycleSHMIntHashIndex::empty()
{
...
//删除
bool CycleSHMIntHashIndex::erase(long key, unsigned int &value)
{
	unsigned int i = (key & (*m_piHashValue));
	unsigned int j;
