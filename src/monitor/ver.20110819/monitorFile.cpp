...
int getAllmsg(char * statisDate1, char * statisDate2,TOCIQuery *pQryCursor)
{
	...
	try{
		...
		sql.clear();
			...
			sql="select z.orgid,z.ccrmsg,x.errmsg+y.nullmsg from "                                                   ;
			sql+="("                                                                                                 ;
			...
			sql+=" )"                                                                                                ;
			 sql+=" x,"                                                                                              ;
			sql+=" ("                                                                                                ;
			    sql+=" select  b.org_id orgid,count(*) nullmsg"                                                      ;
			    sql+=" from  abm_result a,abm_origin_host_tmp b"                                                     ;
			    sql+=" where  a.info_flag='R' "                                                                      ;
			    sql+=" and a.session_id not in"                                                                      ;
			    sql+=" ("                                                                                            ;
			        sql+=" select session_id From abm_result "                                                       ;
			        sql+=" where info_flag is null "                                                                 ;
			        sql+=" and RESULT_DATE > to_date(:p0,'yyyymmddhh24miss')"                                      ;
			        sql+=" and RESULT_DATE <=to_date(:p1,'yyyymmddhh24miss')"                                      ;
			    sql+=" )"                                                                                            ;
			    sql+=" and substr(dest_realm,0,3)=b.ORG_ID "                                                         ;
			    sql+=" and a.RESULT_DATE > to_date(:p0,'yyyymmddhh24miss')"                                        ;
			    sql+=" and a.RESULT_DATE <=to_date(:p1,'yyyymmddhh24miss')"                                        ;
			    sql+=" group by  b.org_id"                                                                           ;
			sql+=" ) "                                                                                               ;
			...
			sql+=" ("                                                                                                ;
			    ...
			    sql+=" where substr(c.DEST_REALM,0,3) = b.ORG_ID"                                                    ;
			    sql+=" and c.info_flag = 'R' "                                                                       ;
			    sql+=" and c.RESULT_DATE > to_date(:p0,'yyyymmddhh24miss')"                                        ;
			    sql+=" and c.RESULT_DATE <=to_date(:p1,'yyyymmddhh24miss')"                                        ;
			    sql+=" group by  b.org_id"                                                                           ;
			sql+=" ) z"                                                                                              ;
			sql+=" where z.orgid=x.orgid(+) and z.orgid=y.orgid(+)"                                                  ;
