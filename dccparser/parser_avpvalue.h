...
{
  public:
    int Utf8ToIso89(char * sSrc, char * sDest)
   {
	  char s_strBufOut[1024];
	  ...
	  iconv_t c_pt;
#ifdef OS_AIX 
	  if ((c_pt = iconv_open("gb18030", "utf8")) == (iconv_t)-1)
#else	
	  if ((c_pt = iconv_open("GB18030", "UTF-8")) == (iconv_t)-1) 
#endif
	  {
		  //Log::log(0,"iconv_open() failed(errno=%d)", errno);
		  return -1;
	  }
	  iconv(c_pt, NULL, NULL, NULL, NULL);

	  iInputLen = strlen(sSrc)+1;
	  iOutLen = sizeof(s_strBufOut);
	  cpOut = s_strBufOut;
#ifdef DEF_AIX	  
	  iReturn = iconv(c_pt, &sSrc, &iInputLen, &cpOut, &iOutLen);
#else	  
	  iReturn = iconv(c_pt, (const char**)&sSrc, &iInputLen, &cpOut, &iOutLen);
#endif	  

	  
	  if (iReturn == -1)
	  {
	    //Log::log(0,"iconv() failed(errno=%d)", errno);
	    strcpy(sDest,"unknow chinese");
	    iconv_close(c_pt);
	    return -1;
	  }
	  
	  iconv_close(c_pt);
	  memcpy(sDest, s_strBufOut, sizeof(s_strBufOut)-iOutLen);
	  return 0;
    }
    
	int Iso89ToUtf8(char * sSrc, char * sDest)
    {
        char s_strBufOut[1024];
	  ...
	  iconv_t c_pt;
#ifdef OS_AIX 
	  if ((c_pt = iconv_open("utf8", "gb18030")) == (iconv_t)-1)
#else	
	  if ((c_pt = iconv_open("UTF-8", "GB18030")) == (iconv_t)-1) 
#endif
	  {
		  AAA_LOG(LM_ERROR, "iconv_open() failed(errno=%d)", errno);
		  return -1;
	  }
	  iconv(c_pt, NULL, NULL, NULL, NULL);
	  ...
	  if (iReturn == -1)
	  {
	    AAA_LOG(LM_ERROR,"iconv() failed(errno=%d)", errno);
	    strcpy(sDest,"unknow chinese");
	    iconv_close(c_pt);
	    return -1;
	  }
	...
	void parseRawToApp() throw(AAAErrorStatus)
    {
      ...
      e->dataType() = AAA_AVP_UTF8_STRING_TYPE;
      char sIsoDestData[10240]={0};	  
      if (Utf8ToIso89((char *)str.c_str(),sIsoDestData) < 0)
	  {
    ...
    void parseAppToRaw() throw(AAAErrorStatus)
    {
      ...
      AAAErrorStatus st;
	  char sIsoDestData[10240]={0};	  
      if (Iso89ToUtf8((char *)str.c_str(),sIsoDestData) < 0)
	  {
		  int nStrLen = str.length();
		  for (int i=0; i<nStrLen; ++i)
		  {
