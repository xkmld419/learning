...
int packNetCtrl(DccMsgParser *poDcc, StruMqMsg *pSendMsg)
{
	...
	//session-id  
    char c1[40]="";
    char c2[40]="ABM2@001.ChinaTelecom.com;";
    char c3[10]=";0001";
	time_t t;
	t = time(NULL);
	itoa(t,c1,10);
	...
	***/
    ...
    sTmp.assign("ABM2@001.ChinaTelecom.com;1306402306;0004");
    ...
    /*****
    sTmp.assign("201105251021481234567800-00010-000024005999-v1.02.03-010001-Recharge.ABM2@001;");
...
int getDccHead(DccMsgParser *poDcc, ABMCCR *pCCR, int iReqFlag)
{
    string sTemp;
    int iCmdId, iPos;
...
int setDccHead(DccMsgParser *poDcc, ABMCCA *pCCA, int iReqFlag)
{
	string sTemp;
	...
	if (iReqFlag == 0)
	    ...
	    poDcc->setDiameterHeader(272, true, pDccHead->m_iHH, pDccHead->m_iEE);
    poDcc->releaseMsgAcl();
...
int packInfoQryA(DccMsgParser *poDcc, ABMCCA *poSendBuf)
{
    ...
    poDcc->setAvpValue("Service-Information.ResponseId", sTemp);
...
int parserChargeR(DccMsgParser *poDcc, ABMCCR *poCCR)
{
	...
	for (int i=0; i<iNums; i++) {
	    ...
	    //oCond.m_sPayDate=U32Var;   timeerror
...
int AllocateBalanceSvcR(DccMsgParser *poDcc, ABMCCR *poCCR)
{
    ...
    ***/
	...
	for (int i=0; i<iNums; i++) {
	    ...
	    snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Request-Debit-Acct-Item[%d].Billing-Cycle", i);
	    poDcc->getAvpValue(sTmpName, iTmp);
...
int AllocateBalanceSvcA(DccMsgParser *poDcc, ABMCCA *poCCA)
{
   ...
   // poDcc->setAvpValue("Service-Information.Balance-Amount ", v1[0]->m_acctBalance);
	...
	for (int i=0; i<v2.size(); i++) {
		 poDcc->setAvpValue("Service-Information.Request-Debit-Acct-Item[%d].Billing-Cycle", v2[i]->m_billingCycle);
		 ...
		 snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Balance-Information.Balance-Item-Detail[%d].Acct-Balance–deduct-Amount", i);
		 var = v2[i]->m_deductAmount;
        poDcc->setAvpValue(sTmpName, var);
		 snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Balance-Information.Balance-Item-Detail[%d].Acct-Balance-Amount", i);
...
int ReqReverseDeductSvcR(DccMsgParser *poDcc, ABMCCR *poCCR)
{
    ...
    if (poDcc == NULL || poCCR == NULL) {
		...
		//oCond.m_sRequestTime = iTmp;     timeerror
	    iTmp -= SECS_70_YEARS;
	    nowtime = (time_t)iTmp;
	    ptm = localtime(&nowtime);
...
int ReqReverseDeductSvcA(DccMsgParser *poDcc, ABMCCA *poCCA)
{
	...
	***/
    ...
    snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Response-Time", 0);
	sTmp.assign(v1[0]->m_sResponseTime);
    poDcc->setAvpValue(sTmpName, sTmp);
...
int packRvsChargeR(DccMsgParser *poDcc, ABMCCA *poCCR)
{
	...
	sscanf(v1[0]->m_sRequestTime, "%04d%02d%02d%02d%02d%02d", 
	                     &tmVar.tm_year, &tmVar.tm_mon, &tmVar.tm_mday,
	                     &tmVar.tm_hour, &tmVar.tm_min, &tmVar.tm_sec);
    tmVar.tm_year -= 1900;
    tmVar.tm_mon -= 1;
...
int parserRvsChargeA(DccMsgParser *poDcc, ABMCCR *poCCA)
{
    ...
    poDcc->getAvpValue("Service-Information.Response-Time", sTmp);
...
int RechargeRecQueryA(DccMsgParser *poDcc, ABMCCA *poCCA)
{
	...
	for (int i=0; i<v2.size(); i++) {
	    snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Order-Info[%d].Result", i);
		poDcc->setAvpValue(sTmpName, v2[i]->m_uiResult);
		
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Operate-Action[%d].Result", i);
		poDcc->setAvpValue(sTmpName, v2[i]->m_uiOperAct);
		 
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Order-Info[%d].RechargeTime", i);
		 sTmp.assign(v2[i]->m_sRechargeTime);
	     poDcc->setAvpValue(sTmpName, sTmp);
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Order-Info[%d].Destination-Account", i);
		 sTmp.assign(v2[i]->m_sRechargeTime);
	     poDcc->setAvpValue(sTmpName, sTmp);
		 
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Operate-Action[%d].Destination-Attr", i);
		...
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Operate-Action[%d].Charge-Type", i);
		poDcc->setAvpValue(sTmpName, v2[i]->m_uiChargeType);
		
		snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Micropayment-Info.Operate-Action[%d].RechargeAmount", i);
...
int packQryBalanceR(DccMsgParser *poDcc, ABMCCA *poCCR)
{
    int iNums;
...
{
    vector<A_ResPlatformSvc *> v1;
    vector<A_ResPlatformPayInfo *> v2;
	...
	//iRet = pIn->getResPlatformPayInfo(v1[0], v2);
    ...
    //time
    poDcc->setAvpValue("Service-Information.Response-Time", v1[0]->m_responseTime);
    //poDcc->setAvpValue("Service-Information.Response-Time", 21023232);
	
	
		 poDcc->setAvpValue("Service-Information.Micropayment-Info.UnitType-Id", v2[0]->m_iUnitTypeId);
		 poDcc->setAvpValue("Service-Information.Micropayment-Info.Deduct-Amount", v2[0]->m_lDeductAmount);
		 
		 iRet = pIn->getResPlatformAcctBalance(v2[0], v3);
		 for (int i=0; i<v3.size(); i++) {
		 poDcc->setAvpValue("Service-Information.Micropayment-Info.Balance-Item-Detail[%d].Balance-Source", v3[i]->m_iBalanceSource);
		 poDcc->setAvpValue("Service-Information.Micropayment-Info.Balance-Item-Detail[%d].Balance-Amount", v3[i]->m_lBalanceAmount);
...
int ReqBalanceSvcA(DccMsgParser *poDcc, ABMCCR *poCCA)
{
    ...
    if (poDcc == NULL || poCCA == NULL) {
	  ...
	  poDcc->getAvpValue("Service-Information.Service-Result-Code", sTmp);
	  poDcc->getAvpValue(sTmpName, iTmp);
	  ...
	  snprintf(oCond.m_sParaFieldResult, sizeof(oCond.m_sParaFieldResult), sTmp.c_str());
	    ...
	    poDcc->getAvpValue("Service-Information.Response-Time", U32Var);
		poDcc->getAvpValue(sTmpName, U32Var);
	  ...
	  /****
	    ...
	    snprintf(oCond.m_responseTime, sizeof(oCond.m_responseTime), "%04d%02d%02d%02d%02d%02d",
                                        ...
                                        ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
	  ****/
	  snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Balance-Unit");
	  poDcc->getAvpValue(sTmpName, iTmp);
	  oCond.m_balanceUnit = iTmp;
	  
	  snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Balance-Amount ");
	  poDcc->getAvpValue(sTmpName, iTmp64);
	  oCond.m_balanceAmount = iTmp64;
	  
	  snprintf(sTmpName, sizeof(sTmpName), "Service-Information.Balance-Type");
	  poDcc->getAvpValue(sTmpName, iTmp);
...
int ReqBalanceSvcR(DccMsgParser *poDcc, ABMCCA *poCCR)
{
	...
	***/
	...
	poDcc->setAvpValue("Service-Information.Recharge-Amount", v1[0]->m_rechargeAmount);
