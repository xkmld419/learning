...
void packChargeR(DccMsgParser *poDcc)
{
    ...
    time_t tTime;
    tTime = time(NULL);
    snprintf(sTmpName1,sizeof(sTmpName1),"%s%010d\0","00220110615164230",tTime);
    sVar1.assign(sTmpName1);
    
    //sVar.assign("002201106151033450057102249");
    poDcc->setAvpValue("Service-Information.Request-Id", sVar1);
   ...
   cout<<"请输入用户号码"<<endl;
    cin>>sTmpName2;
    sVar2.assign(sTmpName2);

    //sVar.assign("charge-20110513000001");
    poDcc->setAvpValue("Service-Information.Micropayment-Info.Destination-Id", sVar2);
    iVar = 2;
    poDcc->setAvpValue("Service-Information.Micropayment-Info.Destination-Id-Type", iVar);

    iVar = 2;
    poDcc->setAvpValue("Service-Information.Micropayment-Info.Destination-Attr", iVar);
    
   	iVar = 001;
    poDcc->setAvpValue("Service-Information.Micropayment-Info.Service_Platform_ID", iVar);
...
int main(int argc,char* argv[])
{
    ...
    snprintf(sTmp,sizeof(sTmp),"%s%03d%s\0","PayStatus_Query.Micropayment@",iVar,".ChinaTelecom.com");
    sVar.assign(sTmp);
