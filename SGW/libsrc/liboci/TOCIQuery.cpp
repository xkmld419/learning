...
TOCIException::TOCIException(sword errNumb, OCIError *hErr, char *cat, char *sql)
{
	int nLen;
	sb4 errcode;
...
{
	if(fErrorNo == OCI_ERROR )
		//THROW(MBC_SR_DataBase+5,"%s",hDBErr);
		throw TOCIException(fErrorNo, hDBErr, "Oracle OCI Call Error","OCIDatabase");
...
bool TOCIDatabase::connect()
{
	...
	OCISessionBegin (hDBSvc, hDBErr, hUser, OCI_CRED_RDBMS, OCI_DEFAULT);
...
void TOCIQuery::getFieldsDef()
{
	...
	if (fFieldCount >0 )
	{
		delete[] fieldList;
		/*
		 modified by 姜明俊 2007-12-25 参数清空请错了
		*/
		//paramList = NULL;
		fieldList = NULL;
	}
...
{
	if (! db->fConnected)
		//THROW(MBC_SR_DataBase+1,"%s--usr=%s;pwd=%s;tns=%s","数据库连接失败，请检查用户名和密码",db->usr,db->pwd,db->tns);	//登录名
...
TOCIParam *TOCIQuery::paramByIndex(const long iIndex)
{
	TOCIParam *para = NULL;
	
	if (fSqlStmt == NULL)
		throw TOCIException(fSqlStmt, ERR_GENERAL, "paramByIndex(): sql statement is empty.");
	
	if(iIndex<fParamCount)
	{
		para = &paramList[iIndex];
  }
	else 
		throw TOCIException(fSqlStmt, ERR_PARAM_NOT_EXISTS, iIndex);
	return para;
}
...
//add by Liaogd 2007/12/26
void TOCIQuery::setParameter(const long iPos, const char* paramValue, bool isOutput )
{
	TOCIParam *para = paramByIndex(iPos); //在paramByIndex中已经有判断参数不存在抛出异常
	para->fIsOutput = isOutput;
	para->dataType = SQLT_STR;
	if (para->stringValue)
		delete[] para->stringValue;
...
void TOCIQuery::setParamArray(char *paramName, char ** paramValue,int iStructSize,int iStrSize,  bool isOutput)
{
	TOCIParam *para = paramByName(paramName); //在paramByName中已经有判断参数不存在抛出异常
...
void TOCIQuery::setParamArray(char *paramName, int * paramValue, int iStructSize, bool isOutput )
{
	TOCIParam *para = paramByName(paramName); //在paramByName中已经有判断参数不存在抛出异常
...
void TOCIQuery::setParamArray(char *paramName, double * paramValue, int iStructSize,bool isOutput) 
{
	TOCIParam *para = paramByName(paramName); //在paramByName中已经有判断参数不存在抛出异常
...
void TOCIQuery::setParamArray(char *paramName, long * paramValue, int iStructSize, bool isOutput )
{
	TOCIParam *para = paramByName(paramName); //在paramByName中已经有判断参数不存在抛出异常
...
void TOCIQuery::setParamArray(char *paramName, void ** paramValue,int iStructSize,int iStrSize,  bool isOutput)
{
	TOCIParam *para = paramByName(paramName); //在ParamByName中已经有判断参数不存在抛出异常
...
TOCIField& TOCIQuery::field(char *fieldName)
{
	int i;
	bool found = false;
...
TOCIParam& TOCIQuery::param(long index)
{
	if (fSqlStmt == NULL)
...
int TOCIQuery::get_fFieldCount() const
{
    return fFieldCount;
}
...
int TOCIQuery::get_fParamCount() const
{
    return fParamCount;
}
...
ub2 TOCIQuery::get_fStmtType() const
{
    return fStmtType;
}
