...
SHMAccess(shmname), m_piHashValue(0), m_piTotal(0), m_piUsed(0), m_poHash(0),m_piIndexTime(0)
{
	if (m_poSHM)
		open ();
}
...
SHMAccess(shmkey), m_piHashValue(0), m_piTotal(0), m_piUsed(0), m_poHash(0),m_piIndexTime(0)
{
	if (m_poSHM)
		open ();
}
...
//删除
void SHMIntHashIndex::erase(long key)
{
	unsigned int i = (key & (*m_piHashValue));
	unsigned int j;
...
void SHMIntHashIndex::open()
{
	m_piHashValue = (unsigned int *)((char *)(*m_poSHM)) + 1;
        m_piIndexTime = (unsigned long *)(m_piHashValue-1);
        m_piHashValue = (unsigned int*)(m_piIndexTime +1);

		m_pimemAdr = (unsigned long *)(m_piHashValue+1+1);
		m_piTotal = (unsigned int*)(m_pimemAdr +1);
////	m_piIndexTime = (unsigned long *)(m_piTotal + 1);
	m_piUsed = (unsigned int*)(m_piTotal +1);
	m_poHash = (m_piUsed + 1);
	//m_poList = (SHMIntList *)(m_poHash + (*m_piHashValue) + 1 ) - 1;
   m_poList = (SHMIntList *)(m_poHash + ((*m_piHashValue)/8)*8 + 8);
}
...
void SHMIntHashIndex::empty()
{
		...
		memset (m_piIndexTime, 0, sizeof (m_piIndexTime));//wangs add
