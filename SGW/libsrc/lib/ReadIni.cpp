...
{
	FILE *fd;
	char sBuffer[LINEMAXCHARS], sSect[LINEMAXCHARS], *p , sTmpBuffer[128];
	int  nAct, nLen;
	char *pTmp;
...
long ReadIni::readIniLong( char const *IniFile, char const *Section, 
char const *Ident, long defint )
{
	char 	defstr[LINEMAXCHARS], sItem[LINEMAXCHARS], *ptr;

	sprintf( defstr, "%ld", defint );
	ptr = readIniString( IniFile, Section, Ident, sItem, defstr );
	if ( ptr == NULL )
		return( defint );
	else
