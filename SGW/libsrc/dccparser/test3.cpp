...
mrtest(unsigned char *buf, int size)
{
  ...
  HeaderParser hp1;
  cout << "start mrtest ..." << endl;
  aBuffer = AAAMessageBlock::Acquire((char*)buf, size);
  hp1.setRawData(aBuffer);
  hp1.setAppData(&msg1.hdr);
  hp1.setDictData(PARSE_LOOSE);
  ...
  print_header(msg1.hdr);

  PayloadParser pp;
  aBuffer->size(msg1.hdr.length);
  pp.setRawData(aBuffer);
  pp.setAppData(&msg1.acl);
  pp.setDictData(msg1.hdr.getDictHandle());
  pp.setListNum(1);
  try {
    pp.parseRawToApp();
  }
  ...
  PayloadParser pp1;
  aBuffer->size(msg.hdr.length);
  pp1.setRawData(aBuffer);
  pp1.setAppData(&msg.acl);

      AAA_IdentityAvpContainerWidget destHostAvp(msg1.acl);
      diameter_identity_t *destHost = destHostAvp.GetAvp("Destination-Host");
      if ( destHost ) 
      {
  	AAACommand *com;
  	printf("Destination-Host=%s\n", destHost->data());
        if( (com = AAACommandList::instance()
         ->search(msg.hdr.code, msg.hdr.appId, msg.hdr.flags.r, -1, destHost->data())) == NULL)
        {
           AAA_LOG(LM_ERROR, "command (%d,r-flag=%1d,scenario=%s) not found\n", 
	       msg.hdr.code, msg.hdr.flags.r, destHost->data());
    	 msg.acl.releaseContainers();
    	 aBuffer->Release();
         exit(1);
        }
  ...
  pp1.setDictData(msg.hdr.getDictHandle());
  try {
    pp1.parseRawToApp();
  }
  catch (AAAErrorStatus st) {
