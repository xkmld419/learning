...
{
  // allocates an AAAMessage in an auto_ptr<>
  AAA_MsgWidget msg(0); 

  // create a checked message parser
  AAA_DiameterMsgParserWidgetChecked parser;
  if (parser.ParseRawToApp(msg, buf, size, PARSE_STRICT) < 0) {
      exit (0);
  }
...
stest(unsigned char *buf, int size)
{
  ...
  //AAADiameterHeader h(1, 0, flag, 257, 0, 0, 0);
  AAA_MsgWidget msg(257);

  // the following widgets allocates AVP containers
  AAA_IdentityAvpWidget originHost("Origin-Host");
  AAA_IdentityAvpWidget originRealm("Origin-Realm");
  AAA_AddressAvpWidget  hostIp("Host-IP-Address");
  AAA_UInt32AvpWidget   vendorId("Vendor-Id");
  AAA_Utf8AvpWidget     product("Product-Name");
  AAA_UInt32AvpWidget   originStateId("Origin-State-Id");
  ...
  AAA_DiameterMsgParserWidgetChecked parser;
  if (parser.ParseAppToRaw(msg, buf, size, PARSE_STRICT) < 0) {
      exit (0);
  }
