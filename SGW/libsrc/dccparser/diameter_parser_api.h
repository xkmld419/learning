...
class DIAMETERPARSER_EXPORT AAAErrorStatus
{
   private:
      ...
      int avpcode;      /**< errornous AVP Code */
   public:
      /*!
       * constructor
       */
      ...
      void set(int type, int code,std::string avp);
      void set(int type, int code,std::string avp,int avpcode);
      void get(int &type, int &code, std::string &avp,int &avpcode);      
...
//typedef ACE_Malloc<ACE_LOCAL_MEMORY_POOL, ACE_SYNCH_NULL_MUTEX> AAAMalloc;
typedef ACE_Allocator_Adapter<ACE_Malloc> AAAAllocator;
...
class DIAMETERPARSER_EXPORT AAAMemoryManager_S : public AAAAllocator
{
//  friend class ACE_Singleton<AAAMemoryManager, ACE_Recursive_Thread_Mutex>; /**< memory manager */
//  friend class ACE_Singleton<AAAMemoryManager, ACE_Null_Mutex>; /**< memory manager */

 public:
   static AAAMemoryManager_S * instance()
	 {
     if( instance_ == 0 )
       instance_ = new AAAMemoryManager_S;
     return instance_ ;
   }
 ...
 private:
   ...
   * constructor
   */
   AAAMemoryManager_S() : AAAAllocator(AAA_MEMORY_MANAGER_NAME) {}
   static AAAMemoryManager_S *instance_;
};

//typedef ACE_Singleton<AAAMemoryManager, ACE_Recursive_Thread_Mutex> AAAMemoryManager_S;
//typedef ACE_Singleton<AAAMemoryManager, ACE_Null_Mutex> AAAMemoryManager_S;
//DIAMETERPARSER_SINGLETON_DECLARE(ACE_Singleton, AAAMemoryManager, ACE_Recursive_Thread_Mutex);
//DIAMETERPARSER_SINGLETON_DECLARE(ACE_Singleton, AAAMemoryManager, ACE_Null_Mutex);
...
class DIAMETERPARSER_EXPORT_ONLY AAAParser
{
   public:
       ...
       * constructor
       */
      AAAParser() :listNum(4), type(0), code(0){ }
      ...
      void     setListNum(int num) { listNum = num; }
      ...
      void getErrorStatus(int& type, int& code)
      {
        type = this->type;
        code = this->code;
      }

      void setErrorStatus(int type, int code)
      {
        this->type = type;
        this->code = code;
      }
...
typedef AAAParser<AAAMessageBlock*, AAAAvpContainerList*, AAADictionaryHandle*>
PayloadParser;
...
template<>
void DIAMETERPARSER_EXPORT_ONLY HeaderParser::parseRawToApp();// throw(AAAErrorStatus);
 ...
 */
template<>
void DIAMETERPARSER_EXPORT_ONLY HeaderParser::parseAppToRaw();// throw(AAAErrorStatus);
 ...
 */
template<>
void DIAMETERPARSER_EXPORT_ONLY PayloadParser::parseRawToApp();// throw(AAAErrorStatus);
 ...
 */
template<>
void DIAMETERPARSER_EXPORT_ONLY PayloadParser::parseAppToRaw();// throw(AAAErrorStatus);
...
class DIAMETERPARSER_EXPORT AAADiameterHeader
{
   ...
   public:
      ...
      void setDictHandle(AAADictionaryHandle* handle) { dictHandle = handle; }
...
typedef AAAParser<AAAMessageBlock*, AAAAvpContainerEntry*,
		  AAADictionaryEntry*> AvpValueParser;
 ...
 */
template<>
void DIAMETERPARSER_EXPORT_ONLY AvpValueParser::parseRawToApp();// throw(AAAErrorStatus);
 ...
 */
template<>
void DIAMETERPARSER_EXPORT_ONLY AvpValueParser::parseAppToRaw();// throw(AAAErrorStatus);
...
class AvpContainerEntryCreator
{
   ...
   */
    AvpContainerEntryCreator(): created_flag(0)
    {

    }

    AAAAvpContainerEntry* operator()(int type)
    {
    	if( !created_flag )
    	{
    	  e = new T(type);
...
class DIAMETERPARSER_EXPORT AvpType {
 public:
  ...
  AvpValueParser* createParser(int type);
...
//    AAAAvpContainerEntry* avpContainerEntry;
 ...
 *  defined.
 */
class DIAMETERPARSER_EXPORT AvpTypeList : public std::list<AvpType*>
{
//  friend class ACE_Singleton<AvpTypeList_S, ACE_Recursive_Thread_Mutex>; /**< type list */
//  friend class ACE_Singleton<AvpTypeList_S, ACE_Null_Mutex>; /**< type list */
 public:
 	  static AvpTypeList * instance()
	  {
      if( instance_ == 0 )
        instance_ = new AvpTypeList;
      return instance_ ;
    }
  /*!
...
//    mutex.release();
 ...
 private:
   ...
   */
  AvpTypeList(void) { registerDefaultTypes(); };
  ...
  static AvpTypeList *instance_;

//  ACE_Null_Mutex mutex; /**< mutex protector */
};
...
//typedef ACE_Singleton<AvpTypeList_S, ACE_Null_Mutex> AvpTypeList;
//DIAMETERPARSER_SINGLETON_DECLARE(ACE_Singleton, AvpTypeList_S, ACE_Null_Mutex);
...
class UTF8Checker
{
 public:
  ...
  int operator()(const char *data, unsigned length, bool nullCheck=false)
  {
	...
	int count1=0;
	for (count1=0; count1<5; count1++) { if ((b && 0x40) == 0) break; }

	// The count value must be greater than 0.
	if (count1==0) { return -1; } // Out of UTF8 character range.
...
{
   public:
     static AAALogMsg * instance()
	   {
	   	 static AAALogMsg *instance_ = 0;
       if( instance_ == 0 )
         instance_ = new AAALogMsg;
       return instance_ ;
     }
		 int	log( Log_Priority priority, const char *format, ... )
		 {
			 va_list args;
		 	 ...
		 	 if (hInstance)
		 	 {
				GetModuleFileName(hInstance,s_File,sizeof(s_File));
				s_File[strlen(s_File)-4]=0;
				strcat(s_File,".log");
			 }
			 else
			 {
				sprintf(s_File,"C:\\AAALogMsg.log");
			 }
		 	 if( (fp=fopen(s_File, "a+")) != NULL ) {
...
#define AAA_LOG AAALogMsg::instance()->log
...
class AAA_MsgWidget
{
    public:
        AAA_MsgWidget(int code,
           ...
           memset(&(m_Msg->hdr), 0, sizeof(m_Msg->hdr));
...
//ACE_UINT32 ntohl32(ACE_UINT32);
void setDictHandle(AAAMessage &msg, const char *requestType, const char *serviceType, const char *serviceId);
void setResultCode(int request, ACE_UINT32 resultCode);
void setRequestFlag(int request);
void setAvpCheck(int avpCheckFlag);
int  getAvpCheck();
void RemoveDictionary();
//void hl32tobuf(ACE_UINT32 int32, char *buf1);
boolean_t getCommandCode(const char *commandName, AAACommandCode& commandCode, AAAApplicationId& appId);
extern std::map<std::string, AAA_AVPCode> MyAvpCodeMap;
