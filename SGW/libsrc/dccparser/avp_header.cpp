...
//
  ...
  {
    AAAErrorStatus st;
    AAADictionaryEntry* avpe;
    avpe = AAAAvpList::instance()->search(h->code, h->vendor);
    AAA_LOG(LM_ERROR, "invalid avpname[%s]----avp[%d] length:[%d] ,len=%d\n",avpe->avpName.c_str(),h->code, h->length, len);
//    st.set(NORMAL, AAA_INVALID_MESSAGE_LENGTH);
    st.set(NORMAL, AAA_INVALID_AVP_LENGTH,avpe->avpName,h->code);
    throw st;
  }
...
//  AAA_LOG(LM_ERROR, "invalid avp code:[%d]\n", h->code);
  ...
  if( getAvpCheck() && h->flag.m == 1 && AAAAvpList::instance()->avpCodeHashmap.find(h->code ) == AAAAvpList::instance()->avpCodeHashmap.end()) 
  {
    ...
    st.set(NORMAL, AAA_AVP_UNSUPPORTED,"",h->code);
    throw st;
  }
