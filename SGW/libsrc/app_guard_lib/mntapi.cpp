...
static CSemaphore *pSemaphore = NULL;
...
int AddTicketStat (int iType, int iNum)
{
	...
	if(!pSemaphore)
	{
		long key =  IpcKeyMgr::getIpcKey(-1, "TICKET_STAT");
		char sKey[32] = {0};
		sprintf(sKey,"%ld",key);
...
int AddThruput (int iNum)
{
    ...
    pCurProc->iThruput = iNum;
    
    pSemaphore->V();
    return pCurProc->iThruput;
}
*/
...
void TStatLogMgr::addRemainBEX (int _iFileID, StdEvent *pEvt, int iCnt)
{
	...
	if(NULL == pEvt) 
		...
		iFromProcessID = Process::m_iFromProcID;
